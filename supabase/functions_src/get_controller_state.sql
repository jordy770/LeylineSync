-- supabase/functions_src/get_controller_state.sql
-- CANONICAL current definition (new in mig 370).
-- Edit THIS file, then generate a migration with scripts/new-migration.mjs.
--
-- One round-trip for the whole controller view. Bundles every read the controller
-- hook (lib/game/use-controller-game-state.ts) used to make as ~19 separate
-- PostgREST requests into a single jsonb payload, so a long game stops accumulating
-- request volume (each game action previously refetched all ~19). The client
-- derives pump totals / protection / status / granted keywords / cost reductions /
-- cast-from-top / play-from-exile from the single raw `continuous_effects` array
-- (same transforms as the old per-effect-type queries).
--
-- SECURITY: security definer (bypasses RLS), so it is locked to the CALLER reading
-- their OWN controller state in a session they belong to — private zones (hand,
-- mana) are owner-scoped to p_player_id = auth.uid().
