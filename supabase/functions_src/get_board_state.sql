-- supabase/functions_src/get_board_state.sql
-- CANONICAL current definition (new in mig 371).
-- Edit THIS file, then generate a migration with scripts/new-migration.mjs.
--
-- The big-screen board view in ONE round-trip — the board counterpart of
-- get_controller_state (mig 370). Bundles the ~8 reads useBoardGameState made per
-- refresh into a single jsonb. No player param and no private zones: the board
-- shows public, all-players battlefield state. status_effects is narrowed to the
-- two types the board badges (animated lands + attack taxes).
--
-- SECURITY: security definer, gated to a session member (auth.uid()); everything
-- returned is data a session member can already read via RLS.
