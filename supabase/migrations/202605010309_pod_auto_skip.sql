-- 202605010309_pod_auto_skip
-- Server-side pod auto-skip: collapse a whole empty-stack priority round into a
-- single pass_priority call instead of one client round-trip per opponent. The
-- engine needs to know each player's auto-pass intent, so this persists it on
-- game_session_players.autopass_settings (a {op,own,stk,rsp} blob the controller
-- syncs from localStorage via set_autopass_settings). pass_priority then chains
-- the passes for downstream players whose settings say they'd auto-pass here too.
--
-- Default '{}' = unknown intent → never auto-skipped (safe: falls back to the
-- per-client behavior). See supabase/functions_src/pass_priority.sql (canonical).
-- (IDE T-SQL diagnostics false-positive on $$ bodies — ignore.)

-- ── Persisted per-player auto-pass intent ────────────────────────────────────
alter table public.game_session_players
  add column if not exists autopass_settings jsonb not null default '{}'::jsonb;

-- ── set_autopass_settings — the controller syncs the caller's own blob up ─────
create or replace function public.set_autopass_settings(
  p_session_id uuid,
  p_settings jsonb
)
returns void
...
begin
  ...
  set autopass_settings = coalesce(p_settings, '{}'::jsonb)
  where session_id = p_session_id
    and player_id = auth.uid();
end;
$$;

grant execute on function public.set_autopass_settings(uuid, jsonb) to authenticated;

-- ── pass_priority (canonical: functions_src/pass_priority.sql) ────────────────
