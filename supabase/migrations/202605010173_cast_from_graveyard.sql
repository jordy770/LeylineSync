-- Cast-from-graveyard permission (Liliana, Untouched by Death's -3:
-- "You may cast Zombie spells from your graveyard this turn.")
--
-- Three contained changes, all reusing existing machinery:
--   1. A new continuous-effect type `cast_from_graveyard` — a player-scoped, type-
--      filtered permission that expires at this turn's cleanup step (ending/cleanup),
--      swept by the existing expire_continuous_effects_for_step. It is NOT flagged
--      registered_from_card_script, so rebuild_scripted_continuous_effects leaves it.
--   2. A `grant_cast_from_graveyard` branch in apply_triggered_ability_effects
--      (reproduced from mig 171) that inserts that permission row.
--   3. cast_card_from_hand (reproduced from mig 134) now also accepts a source in the
--      graveyard, but ONLY when a matching permission exists for the caster + card type.
-- (IDE T-SQL false-positives on $$ bodies — ignore.)

-- 1. Allow the new effect type on the continuous-effects table (reproduced from mig 159).
...
alter table public.game_continuous_effects
  ...
  check (effect_type = any (array[
    ...
    'infect', 'wither', 'toxic', 'cast_from_graveyard'
  ]));

-- 2. apply_triggered_ability_effects (reproduced from mig 171) + grant_cast_from_graveyard.
  ...
  loop
    ...
    if v_eff_type = 'gain_life' then
      ...
      if v_eff_amount > 0 then
        ...
        foreach v_rid in array coalesce(v_recipients, array[]::uuid[]) loop
          if v_rid is not null then
            for v_draw_i in 1..v_eff_amount loop
              ...
              order by zone_position asc, id asc limit 1 for update skip locked;
              exit when v_lib_card is null;
              -- "If a <type> card is milled this way": note a match before it moves.
        ...
        -- Conditional follow-up (applied once, source as controller).
      ...
      -- A player-scoped permission to cast cards of a given (sub)type from the
      -- graveyard, expiring at this turn's cleanup. type_line '' means "any card".
      if p_controller_id is not null then
...
-- 3. cast_card_from_hand (reproduced from mig 134) now also accepts a graveyard source
--    when a cast_from_graveyard permission covers the card's type.
