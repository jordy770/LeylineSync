-- "Sacrifice a creature" as an activated-ability cost (Spark Reaper: "{2}{B},
-- Sacrifice another creature: Draw a card"; Vampiric Rites: "{1}{B}, Sacrifice a
-- creature: Draw a card and lose 1 life"). The sacrificed creature is passed as
-- p_target_card_id (the ability's effect is untargeted).
--
-- Also generalizes the effect dispatch: a MULTI-effect untargeted ability (draw +
-- lose life) resolves its whole program via a spell_effect stack item, instead of
-- the single-effect-per-branch path (single-effect abilities are unchanged).
--
-- Reproduced from migration 178 with: declare v_has_sac_creature; a
-- 'sacrifice_creature' cost branch + its validation/payment; and the multi-effect
-- spell_effect route.
