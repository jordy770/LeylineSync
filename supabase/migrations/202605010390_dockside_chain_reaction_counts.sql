-- 202605010390_dockside_chain_reaction_counts
-- Two new resolve_count_amount counts (Prosper precon, bulk-pipeline batch):
--   * 'opponent_artifacts_and_enchantments' — Dockside Extortionist's ETB
--     Treasure count ("artifacts and enchantments your opponents control").
--   * 'creatures_on_battlefield' — Chain Reaction's X ("each creature", all
--     players). deal_damage_all already resolves its amount through
--     resolve_dynamic_amount; the Zod schema gains the CountAmountSchema union.
