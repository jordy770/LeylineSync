-- 202605010339_reflexive_may_program
-- Reflexive "when you do" via may + program (Ruthless Lawbringer: "you may
-- sacrifice another creature. When you do, destroy target nonland permanent").
-- apply_trigger_effects: the `may` confirm now carries a `program` flag, and
-- destroy_up_to gains a `nonland` target filter. submit_decision: on confirming a
-- program-may, the inner effects run as a fresh enqueued triggered ability (the
-- sacrifice edict parks first, then destroy_up_to parks its own target pick), so
-- the destroy target is chosen AFTER the sacrifice. Gate the may with a count
-- condition so it is only offered when a sacrifice is possible.
