-- 202605010261_dino_fights
-- Veloci-Ramp-Tor fights batch (4 cards): Savage Stomp, Wayta Trainer
-- Prodigy, Scion of Calamity, Progenitor's Icon (script-only).
--   • New 'fight_pick' program action + decision: the program's target
--     ("target creature you control") fights a SECOND parked pick via
--     apply_fight. Used by Savage Stomp (spell) and Wayta (activated; the
--     single-effect routed list now carries the activation target).
--     Approximations: Savage Stomp's conditional {2} discount and Wayta's
--     both-yours discount are not modelled; Wayta's trigger-doubling
--     replacement is NOT modelled.
--   • Per-attacker 'dealt_combat_damage_to_player' event from
--     resolve_combat_damage (regular + spillover), carrying event_amount and
--     event_player_id. Scion of Calamity destroys an artifact/enchantment on
--     connect; destroy_up_to gains a `types` array filter ("artifact or
--     enchantment"). Scion's MYRIAD is NOT modelled (no-op in 1v1).
--   • Progenitor's Icon: any-color mana ability only; the chosen-type flash
--     grant is not modelled (cast timing is not engine-enforced).
-- Generated from supabase/functions_src (apply_trigger_effects, submit_decision, activate_ability, resolve_combat_damage) — those files are
...
begin
  ...
  elsif v_decision.decision_type in ('search_library', 'choose_cards', 'sacrifice', 'return_from_graveyard', 'reanimate_destroyed', 'look_top', 'proliferate', 'copy_permanent', 'become_copy', 'bounce_pick', 'cast_exiled_free', 'put_from_hand_pick', 'destroy_pick', 'command_zone_pick', 'graveyard_exile_pick', 'fight_pick') then
