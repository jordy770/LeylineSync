-- Scourge of Nel Toth — "You may cast this creature from your graveyard by
-- paying {B}{B} and sacrificing two creatures rather than paying its mana
-- cost." Top-level script prop `graveyard_cast_cost: { mana, sacrifice_creatures }`:
-- a card carrying it casts from its owner's graveyard WITHOUT a permission row;
-- the alternative mana replaces the printed cost and the sacrifices are paid at
-- cast (p_sacrifice_ids = the caster's chosen set; engine auto-picks when null).
-- Also counts toward the Laboratory Drudge graveyard-cast tracker.
-- Old 5-arg signature dropped first (defaulted-param ambiguity, bug-421).

drop function if exists public.cast_card_from_hand(uuid, uuid, jsonb, uuid, boolean);
