-- Syphon Flesh — "Each other player sacrifices a creature. Create a 2/2 black
-- Zombie token for each creature sacrificed this way." A sacrifice TALLY threaded
-- through the edict chain: submit_decision accumulates a `sacrificed_count` on the
-- spell's stack item as each opponent sacrifices; a create_token with a dynamic
-- count {count:'sacrificed_this_way'} reads that tally on resume (0 → no tokens).
--
-- Reproduces apply_trigger_effects (mig 170) + submit_decision (mig 179) verbatim
-- with surgical additions only. (IDE T-SQL false-positives on $$ bodies — ignore.)
