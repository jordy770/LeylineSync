-- Amass N (War of the Spark) — "If you don't control an Army, create a 0/0 black
-- Zombie Army creature token. Then put N +1/+1 counters on an Army you control."
-- (Enter the God-Eternals, Eternal Skylord, Lazotep Plating/Reaver, Vizier of the
-- Scorpion, Gleaming Overseer.)
--
--   1. public.amass(session, controller, amount) — find-or-create the Army (a
--      seeded "Zombie Army" 0/0 token) and add N +1/+1 counters. Atomic, so the
--      0/0 never hits an SBA before it has counters.
--   2. apply_triggered_ability_effects — reproduced from mig 174 with one added
--      branch: `amass` → perform public.amass(...).

-- ── 1. The amass helper ──────────────────────────────────────────────────────
create or replace function public.amass(
  p_session_id uuid,
...
begin
  if p_controller_id is null or p_amount <= 0 then
    return;
  end if;

  -- The Army you amass onto (you can control at most one being amassed).
  select gc.id into v_army_id
  ...
  -- None → create a 0/0 black Zombie Army token.
  if v_army_id is null then
    select id into v_token_card_id
    from public.cards
    where lower(name) = 'zombie army' and is_token = true
    limit 1;
    if v_token_card_id is null then
      return; -- token not seeded
    end if;
    ...
    perform public.register_card_continuous_effects(p_session_id, v_army_id);
  end if;

  -- Put N +1/+1 counters on the Army (it grows from 0/0).
  update public.game_cards
  set plus_one_counters = plus_one_counters + p_amount
  where id = v_army_id;
end;
$$;

grant execute on function public.amass(uuid, uuid, integer) to authenticated;

-- ── 2. apply_triggered_ability_effects + amass branch (reproduced from mig 174) ─
...
begin
  ...
  loop
    ...
    if v_eff_type = 'gain_life' then
      ...
      -- Amass N: find-or-create the Army and add N +1/+1 counters.
