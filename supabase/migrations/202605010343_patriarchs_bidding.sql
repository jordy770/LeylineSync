-- 202605010343_patriarchs_bidding
-- Patriarch's Bidding: who:'each_player' on choose_creature_type. apply_trigger_effects
-- queues all players (seat order) and raises the first chooser's decision, carrying
-- the rest in params.player_queue. submit_decision injects $chosen into
-- return_all_from_graveyard (each player reanimates their OWN graveyard's creatures
-- of their chosen type) and raises the next queued player's decision until empty.
