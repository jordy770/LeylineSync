-- 202605010291_commander_cost_reduction
-- Commander casts honour static cost reductions (mig 291): cast_commander
-- paid the PRINTED cost directly, so Nogi / Dragonlord's Servant /
-- Etherium Sculptor / Millicent's affinity never discounted a command-zone
-- cast. The printed cost now runs through reduced_mana_cost (the tax stays
-- a separate unreduced payment — strict rules would let the reduction eat
-- tax once the printed generic is exhausted; documented nuance).
-- Generated from supabase/functions_src (cast_commander) — those files are
