-- Necromantic Selection — "Destroy all creatures, then return a creature card
-- put into a graveyard this way to the battlefield under your control. It's a
-- black Zombie in addition to its other colors and types. Exile ~."
-- New spell-program effect `mass_destroy_reanimate_one`:
--   apply_trigger_effects snapshots the about-to-die set (indestructible
--     excluded), runs destroy_all_creatures, then parks a single-pick decision
--     over the died cards that actually reached a graveyard;
--   submit_decision's `reanimate_destroyed` branch returns the chosen card to
--     the battlefield under the DECIDING player's control (ownership unchanged)
--     — unlike return_from_graveyard, which returns to the owner.
-- NOT modelled (documented gaps): the "black Zombie in addition" type/colour
-- addition (no type-addition layer exists) and the self-exile of the sorcery
-- (it goes to its owner's graveyard like any other spell).
