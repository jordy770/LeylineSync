-- 202605010267_equipment2
-- EQUIPMENT phase 2 (mig 267): Skullclamp, Cranial Plating, Bonehoard,
-- Grip of Phyresis, Armory Automaton (5 cards).
--   • Watcher filter attached_host (Skullclamp: 'whenever equipped creature
--     dies, draw two' fires only for the watcher's own host).
--   • Dynamic continuous pump: payload power_count/toughness_count name a
--     count resolved at READ time against the source's controller
--     (card_layered_power/toughness — newly canonical from mig 209 — gain a
--     third addend). Cranial Plating (+1/+0 per artifact; the
--     attach-at-instant-speed rider is not modelled), Bonehoard (+X/+X =
--     creature cards in ALL graveyards via the new
--     creature_cards_all_graveyards count).
--   • living_weapon action: Germ token + attach the program's target
--     Equipment (Grip of Phyresis, after gain_control) or the source itself
--     (Bonehoard ETB).
--   • attach_all_equipment (Armory Automaton). Approximation: attaches every
--     Equipment YOU control; opponents' Equipment and partial picks are not
--     modelled, and the trigger also fires on attack via the same script.
-- Generated from supabase/functions_src (resolve_count_amount, card_layered_power, card_layered_toughness, fire_watcher_triggers, apply_trigger_effects) — those files are
