-- 202605010242_kessig_chaos_warp — Kessig Wolf Run + Chaos Warp.
--   • activate_ability gains p_x_value: an {X} in the activation cost is paid
--     as that much generic mana and every literal 'X' power/toughness/amount
--     in the effects becomes the chosen value (Kessig Wolf Run: "{X}{R}{G},
--     {T}: Target creature gets +X/+0 and gains trample"). The multi-effect
--     path now carries p_target_card_id on the spell_effect payload, so a
--     targeted multi-effect ability routes each targeted effect to the pick.
--     SIGNATURE CHANGE: the old 6-arg overload is dropped below.
--   • apply_creature_effect gains kind shuffle_into_library (Chaos Warp): the
--     owner shuffles the target into their library (random-position insert),
--     then with then_reveal_top_to_battlefield the revealed top card enters
--     the battlefield if it's a permanent card. Tokens shuffled in cease.
--   • trigger_effect_target_type adds shuffle_into_library to the
--     permanent-targeted family (first canonical edit since mig 172).
-- Generated from supabase/functions_src (activate_ability, apply_creature_effect, trigger_effect_target_type) — those files are
-- the canonical current definitions; edit them, not past migrations.

-- Adding p_x_value changes the signature; drop the old overload so named and
-- positional 6-arg calls resolve to the new function.
drop function if exists public.activate_ability(uuid, uuid, integer, uuid, uuid, jsonb);
...
begin
  ...
  elsif v_eff_type in ('create_token', 'search_library', 'grant_keyword_all', 'return_all_from_graveyard', 'deal_damage_all', 'monstrosity', 'divide_damage', 'return_from_graveyard') then
