-- Cruel Revival — "Destroy target non-Zombie creature. It can't be regenerated.
-- Return up to one target Zombie card from your graveyard to your hand."
--   build_stack_payload_permanent_simple gains `exclude_type_line` (cast-time
--     rejection of matching targets — the first NEGATIVE type restriction) and
--     carries `then_return_from_graveyard` into the payload;
--   handle_permanent_effect, after the removal + existing riders, parks a
--     return_from_graveyard decision for the CASTER (min 0 = "up to one";
--     filter type_line; to hand) — the Assassin's Trophy awaiting_decision
--     contract, finished by the existing submit_decision branch.
-- "It can't be regenerated" is moot (regeneration isn't modelled). The return
-- is decision-picked rather than targeted at cast (consistent with every other
-- graveyard pick in this engine).
-- Generated from supabase/functions_src (build_stack_payload_permanent_simple, handle_permanent_effect) — those files are
