-- 202605010272_ixhel_corrupted
-- Ixhel corrupted batch (mig 272, 9 cards): Ixhel commander, Bojuka Bog,
-- Fumigate, Culling Ritual, Caress of Phyrexia, Carrion Call, Cankerbloom,
-- Contaminant Grafter, Glistening Sphere.
--   • Count opponent_poison_counters (max among opponents) powers corrupted
--     gates via the existing conditional effect (at_least 3).
--   • destroy_all_creatures_token gains gain_per_destroyed (Fumigate).
--   • destroy_all_mv: wipe nonland MV<=N + fixed-colour ritual mana
--     (Culling Ritual — the B-or-G choice is approximated as B).
--   • add_poison (Caress; the target player's DRAW is not modelled and the
--     poison/life land on the opponent).
--   • exile_graveyard (Bojuka Bog: the opponent's graveyard).
--   • ixhel_corrupted_exile: each poisoned opponent exiles their library top
--     with an impulse-style play_from_exile permission for you.
--     Approximations: the play window is until the end of your next turn
--     (not unlimited), face-down hiding and any-colour spending unmodelled.
-- Script-only: Carrion Call (+Phyrexian Insect Token, infect), Cankerbloom
-- (sac modal), Contaminant Grafter (connect-proliferate + corrupted
-- conditional draw/land), Glistening Sphere (corrupted 3-mana mode gate
-- UNENFORCED on the mana ability).
-- Generated from supabase/functions_src (apply_triggered_ability_effects, resolve_count_amount) — those files are
