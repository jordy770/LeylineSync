-- 202605010326_optional_trigger_targets
-- Optional ("up to one target …") triggered-ability targets. The targeting effect
-- may carry optional:true → the trigger is NOT required to target: it enqueues with
-- target_required=false + target_optional=true (keeping target metadata so a picker
-- can still be offered), and resolves as a no-op if no target is chosen — so it can
-- never soft-lock the stack (Obuun's begin-combat animate). apply_trigger_effects
-- skips a targeted effect whose target wasn't chosen instead of applying it null,
-- and choose_triggered_ability_creature_target now accepts optional (not only
-- required) triggers so the chooser/bot can still set a target.
-- Generated from supabase/functions_src (enqueue_triggered_ability, apply_trigger_effects, choose_triggered_ability_creature_target) — those files are
