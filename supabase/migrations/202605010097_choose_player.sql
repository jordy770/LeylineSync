-- Phase 1, slice 10: choose_player — pick a player at resolution, then run inner
-- effects against them ("target player of your choice loses 3", "an opponent you
-- choose draws", ...).
--
-- Clean reuse: instead of teaching apply_triggered_ability_effects about a
-- specific recipient id, submit_decision runs the inner effects with the CHOSEN
-- player as the controller context and recipient forced to 'controller' — so
-- lose_life/deal_damage/gain_life/draw/mill all land on the chosen player. Inner
-- effects must be simple player-directed effects (no nested decision).
--
-- Reproduces apply_trigger_effects (096) + a choose_player park branch, and
-- submit_decision (096) + choose_player validate/apply. (IDE T-SQL false-positives.)
...
-- submit_decision: reproduced from 096 with choose_player validate + apply added.
...
begin
  ...
  if v_decision.decision_type = 'scry' then
    ...
    -- Run the inner effects with the chosen player as the controller context and
    -- recipient forced to 'controller', so they land on the chosen player.
