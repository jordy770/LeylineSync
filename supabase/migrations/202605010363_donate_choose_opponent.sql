-- 202605010363_donate_choose_opponent
-- Donate (Harmless Offering) now lets the caster CHOOSE which opponent gains the
-- permanent in multiplayer (the card reads "target player"). handle_permanent_effect:
-- for kind=gain_control to:opponent with >1 living opponent, park a choose_player
-- decision (options = living opponents) carrying the permanent_effect in params;
-- a single opponent (1v1) still applies directly with no prompt. submit_decision:
-- a choose_player decision with `apply_permanent_effect` runs that effect with the
-- chosen player as acting_controller — a reusable "choose an opponent, then hand
-- them a permanent" hook for future cards.
-- Generated from supabase/functions_src (handle_permanent_effect, submit_decision) — those files are
