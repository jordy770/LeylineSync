...
begin
  ...
  -- creature"): reject a target whose type line matches exclude_type_line. mig 412:
  -- the value may also be a JSON ARRAY of types (Victim of Night: "that isn't a
  -- Vampire, Werewolf, or Zombie") — reject a target matching ANY listed type.
  if p_payload -> 'exclude_type_line' is not null
     and jsonb_typeof(p_payload -> 'exclude_type_line') is distinct from 'null'
     and exists (
    select 1 from public.game_cards gc
    join public.cards c on c.id = gc.card_id
    where gc.id = v_target_card_id and gc.session_id = p_session_id
      and case jsonb_typeof(p_payload -> 'exclude_type_line')
            when 'array' then exists (
              select 1 from jsonb_array_elements_text(p_payload -> 'exclude_type_line') ex
              where nullif(ex.value, '') is not null and c.type_line ilike '%' || ex.value || '%')
            else c.type_line ilike '%' || (p_payload ->> 'exclude_type_line') || '%'
          end
  ) then
    raise exception 'Target is an excluded type for this spell';
  end if;
