-- 202605010392_talrand_finishers
-- Talrand finishers (bulk-pipeline follow-up) — four engine features:
--   * grant_flashback (Snapcaster Mage): ETB parks a graveyard pick; the chosen
--     instant/sorcery gets a turn-stamped counters.flashback_until_turn, which
--     cast_spell_effect's graveyard branch now honors (cost = its mana cost).
--   * hand_to_library_top (Brainstorm): parked mandatory hand pick; each chosen
--     card goes to the library top (last pick ends on top).
--   * unless_pays rider on counter (Mana Leak): handle_counter_spell parks a
--     pay_or_be_countered decision for the TARGET spell's controller; pay = the
--     spell stands, decline = countered.
--   * amount {power_of:'triggering_creature'} (Terror of the Peaks) and
--     play_hideaway to:'hand' (Watcher for Tomorrow) in apply_trigger_effects.
-- Also backfills functions_src/cast_spell_effect.sql with the mig 388
-- adventure-face changes it was missing (the first cut of this migration
-- regenerated from the stale source and reverted them).
-- Generated from supabase/functions_src (apply_trigger_effects, submit_decision, cast_spell_effect, handle_counter_spell) — those files are
