-- Energy as an activation cost (roadmap Counters #8, the consumable half).
--
-- Energy counters are already TRACKED + proliferatable (mig 154) and gainable via
-- add_player_counters, but nothing SPENT them. This adds an `energy` activated-ability
-- cost: "{E}{E}: <effect>" → spend N energy from the activating player's pool. The pool
-- lives on game_session_players.counters->>'energy'; we check affordability and deduct
-- with adjust_counter_bag (clamps at 0). Atomic with the rest of activation, so if any
-- later cost (mana) can't be paid the whole thing rolls back — no partial spend.
--
-- Authoring: a cost entry { "type": "energy", "amount": N } on an activated ability
-- (JSON/AI-authored — the guided builder only does tap/mana, like other engine-only
-- costs). NOT in scope (deferred #8 half 2): reading experience to SCALE an ability's
-- amount (that needs the dynamic-amount machinery deferred from #5 half 2).
--
-- Reproduced from CURRENT (grep-first): activate_ability (mig 119) verbatim except the
-- energy cost parse + check + deduction. (IDE T-SQL false-positives on $$ bodies.)
...
begin
  ...
  if v_eff_type = 'draw' then
    v_stack := public.put_action_on_stack(
      ...
      jsonb_build_object('amount', greatest(1, coalesce((v_effect ->> 'amount')::integer, 1)), 'timing', 'instant'),
      p_source_card_id
    );
