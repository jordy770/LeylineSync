-- 202605010270_breya_tail
-- Breya tail batch (11 cards, mig 270): Beacon of Unrest, Grave Upheaval,
-- Silas Renn, Ethersworn Adjudicator, Faerie Artisans, Soul of New Phyrexia,
-- Magus of the Will, Sydri, Everflowing Chalice, Parting Thoughts,
-- Trial // Error.
-- Engine: return_from_graveyard gains from:'all_graveyards' (the pick spans
-- every graveyard), control:'decider' (enters under the decider), and a
-- haste rider (plain unexpiring row). Serves Beacon of Unrest
-- (artifact-or-creature from any graveyard under you, shuffle-self NOT
-- modelled) and Grave Upheaval (creature from any graveyard with haste;
-- basic landcycling NOT modelled).
-- Script-only approximations: Sydri = only the deathtouch grant (animation
-- and lifelink not modelled); Everflowing Chalice = enters with one charge,
-- taps for {C} (multikicker not modelled); Parting Thoughts = plain destroy
-- (counter-scaled draw/life riders not modelled); Trial // Error = the
-- Error half only (counter target spell); Soul of New Phyrexia = creatures
-- only; Faerie Artisans = no auto-sacrifice of the previous token.
