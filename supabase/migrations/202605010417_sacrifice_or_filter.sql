-- sacrifice_or_filter
-- The `sacrifice` effect filter now accepts type_line_any (OR over several types):
-- "sacrifice an artifact or creature" — Deadly Dispute, Costly Plunder — so a
-- Treasure/Clue or a creature can pay. park_edict_sacrifice (below, migration-only)
-- gains an optional p_filter_any param; the two callers (apply_trigger_effects,
-- submit_decision resume) thread it. Single-type sacrifices are unchanged.
...
-- park_edict_sacrifice gains an optional p_filter_any (mig 417): the sacrifice
-- options match p_filter (single type) OR any type in p_filter_any. The chosen
-- filter_any is stored on the decision so the each-opponent resume chain keeps it.
-- Migration-only function (originally mig 110); default null = unchanged callers.
create or replace function public.park_edict_sacrifice(
  ...
  p_filter text,
  p_queue jsonb,
  p_filter_any jsonb default null
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  while v_idx < v_n loop
    ...
    where gc.session_id = p_session_id and gc.controller_player_id = v_decider and gc.zone = 'battlefield'
      and (
        -- OR-types (mig 417): match any listed type when p_filter_any is given.
        case when jsonb_typeof(p_filter_any) = 'array' then exists (
               select 1 from jsonb_array_elements_text(p_filter_any) ta(t)
               where nullif(ta.t, '') is not null and c.type_line ilike '%' || ta.t || '%')
             when p_filter is not null then c.type_line ilike '%' || p_filter || '%'
    ...
    if v_len > 0 then
      ...
      values (p_session_id, v_decider, p_stack_item_id, 'sacrifice', 'Sacrifice ' || v_amount, v_options, v_amount, v_amount,
        jsonb_strip_nulls(jsonb_build_object('count', coalesce(p_count, 1), 'filter', p_filter,
                          'filter_any', p_filter_any, 'edict_queue', v_remaining)))
      returning id into v_decision_id;
      return v_decision_id;
    end if;
