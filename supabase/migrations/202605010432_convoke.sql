-- 202605010432_convoke
-- Convoke (Hour of Reckoning, Triplicate Spirits, Markov Baron + Chief
-- Engineer's grant): cast_spell_effect and cast_card_from_hand gain a
-- p_convoke_card_ids uuid[] (default null, new last param). The new
-- apply_convoke helper validates the chosen creatures (distinct, untapped,
-- yours, on the battlefield), TAPS them, and rewrites the cost — per creature
-- one matching coloured pip (card_color_set of its mana cost) or {1} generic;
-- over-convoke raises. Spells need `convoke: true` in their script;
-- cast_card_from_hand also accepts a grants_convoke continuous effect with a
-- matching type_line (Chief Engineer: "Artifact spells you cast have convoke"),
-- newly registrable via register_card_continuous_effects.
-- Generated from supabase/functions_src (apply_convoke, cast_spell_effect, cast_card_from_hand, register_card_continuous_effects) — those files are
-- the canonical current definitions; edit them, not past migrations.

-- 'grants_convoke' joins the continuous-effect CHECK list (rebuilt from the
-- mig 407 definition + the new value — bug-283 rule).
...
-- old ones. Drop the old signatures first (same pattern as mig 418/428-431).
drop function if exists public.cast_spell_effect(uuid, jsonb, uuid, integer, uuid, boolean, boolean, boolean, boolean, uuid[]);
drop function if exists public.cast_card_from_hand(uuid, uuid, jsonb, uuid, boolean, uuid[], integer);
