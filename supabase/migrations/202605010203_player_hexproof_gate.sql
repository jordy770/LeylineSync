-- Player hexproof targeting gate (Lazotep Plating — "You and permanents you
-- control gain hexproof until end of turn"). A grant_keyword_all hexproof row
-- with payload.includes_player covers its player(s): put_action_on_stack now
-- rejects a player-targeting action (target_player_id in the built payload)
-- from an OPPONENT while such a grant is active; targeting yourself stays
-- legal. The permanents half already works via the ordinary hexproof accessor
-- (mig 200 mass branch).
