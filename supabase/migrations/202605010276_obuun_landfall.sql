-- 202605010276_obuun_landfall
-- Obuun landfall mega batch (21 cards, mig 276). Engine: counts
-- countered_creatures_you_control (Inspiring Call) + opponent_hand_excess
-- (Sandstone Oracle); return_from_graveyard filter max_mana_value
-- (Sun Titan). Everything else script-only — landfall token suite (Omnath,
-- Baloths, Sporemound, Zendikar's Roil, Khalni Heart), landfall modals
-- (both Retreats), fetch package (Harrow, Roiling Regrowth, Springbloom,
-- Panorama, Kor Cartographer, Yavimaya Elder), outlast bodies (Falconer /
-- Tuskguard — the counter-conditional keyword ANTHEM is not modelled),
-- Sylvan Advocate (6-land conditional self pump), Emeria Shepherd (hand
-- return; the Plains-battlefield rider dropped), Hour of Revelation,
-- Harmonize, Needle Spires (land ANIMATION not modelled).
-- Generated from supabase/functions_src (resolve_count_amount, apply_trigger_effects) — those files are
