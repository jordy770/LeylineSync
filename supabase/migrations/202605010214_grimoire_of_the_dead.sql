-- Grimoire of the Dead — "{1}, {T}, Discard a card: Put a study counter on ~."
-- "{T}, Remove three study counters from ~ and sacrifice it: Put all creature
-- cards from all graveyards onto the battlefield under your control."
--   activate_ability: new costs `discard` (the chosen hand card rides
--     p_target_card_id, which is NULLED once consumed — the effect is
--     untargeted) and `remove_counters` ({counter_type, amount} from the
--     source's bag); untargeted add_counters routes via a spell_effect item
--     (the resolver defaults to the source — study counters land on the
--     Grimoire); return_all_from_graveyard joins the spell_effect route list.
--   return_all_from_graveyard gains p_all_graveyards (sweep every graveyard,
--     cards enter under the CONTROLLER's control); the ATAE branch passes
--     from:'all_graveyards'. Old 4-arg signature dropped (bug-421).
-- NOT modelled: "They're black Zombies in addition…" (no type-addition layer).

drop function if exists public.return_all_from_graveyard(uuid, uuid, text, text);
-- Generated from supabase/functions_src (activate_ability, return_all_from_graveyard, apply_triggered_ability_effects) — those files are
