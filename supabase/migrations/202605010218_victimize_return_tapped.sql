-- Victimize — "Choose two target creature cards in your graveyard. Sacrifice a
-- creature. If you do, return the chosen cards to the battlefield tapped."
-- return_from_graveyard gains a `tapped` option: the park (apply_trigger_effects)
-- rides it into the decision params; submit_decision's battlefield return sets
-- is_tapped from it. Victimize is then the composition
--   [ sacrifice {who:you, count:1}, return_from_graveyard {to:battlefield,
--     count:2, tapped:true} ]
-- Sequencing approximation (documented): the engine sacrifices FIRST, then
-- picks the returns — rules-wise the targets are chosen before the sacrifice,
-- but with no creature to sacrifice the sacrifice step simply skips and the
-- return still parks, slightly more permissive than the printed card.

-- Zod: the return_from_graveyard action gained `tapped` (schema file).
