-- cast_card_free
-- cast_card_free casts a card (currently in exile) for free: permanents get a
-- real cast_permanent stack push (true ETBs on resolution); no-target instants/
...
--
-- Review fix: cast_spell_effect's exile-source play_from_exile gate is now
-- guarded with `not p_free` — a free (engine-authorized) cast is
-- self-authorizing and needs no permission row, mirroring the p_free guard
-- already used on the payment block below it. cast_card_free no longer
-- inserts a `permanent: true` play_from_exile row to get past that gate: that
-- row never expired (advance_step.sql only deletes `permanent = false` rows)
-- and could leak/accumulate, staling-re-authorizing the card for later casts.
-- Generated from supabase/functions_src (cast_spell_effect, cast_card_free) — those files are
