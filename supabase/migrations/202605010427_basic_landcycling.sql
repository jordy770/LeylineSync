-- 202605010427_basic_landcycling
-- Basic landcycling (bucket 2 van de engine-backlog; Ash Barrens, Grave
-- Upheaval, Migratory Route, Treacherous Terrain):
--   * cycle_card krijgt p_landcycle (signatuur 3→4 args, oude overload
--     gedropt): betaal de `landcycling`-cost, discard, en park een STACK-LOZE
--     search_library-decision (opties = basic lands in de eigen library,
--     to:hand, reveal). Geen draw in deze modus.
--   * submit_decision search_library-branch slaat resume_or_finalize over
--     wanneer source_stack_item_id null is (stack-loze decisions).
-- Generated from supabase/functions_src (cycle_card, submit_decision) — those files are
-- the canonical current definitions; edit them, not past migrations.

drop function if exists public.cycle_card(uuid, uuid, jsonb);
