...
begin
  ...
  if v_commander is null then
    -- ::text disambiguates array||element (a bare literal is "unknown" type and
    -- Postgres would otherwise try to parse it as an array → malformed-array error).
    v_issues := v_issues || 'No commander designated'::text;
  end if;
