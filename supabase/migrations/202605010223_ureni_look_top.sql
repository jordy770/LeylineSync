-- Ureni of the Unwritten (the Dragons commander) — "Whenever Ureni enters or
-- attacks, look at the top eight cards of your library. You may put a Dragon
-- creature card from among them onto the battlefield. Put the rest on the
-- bottom of your library in a random order."
--
-- New decision effect `look_top { count, filter:{type_line, creature?}, to }`:
--   apply_trigger_effects looks at the top N, offers the matching cards (a
--     may-pick, min 0 max 1), stashes the looked-at set in params; if nothing
--     matches it bottoms the whole set with no decision.
--   submit_decision puts the chosen card onto the battlefield (fires ETB) and
--     bottoms every other looked-at card in random order.
-- Both reuse the new helper bottom_cards_random. Authored as two triggered
-- abilities (enters_the_battlefield + attacks) on Ureni.
