-- Phase 1, slice 14 — search_library variants beyond the basic tutor.
--
-- Four additions to the existing search_library effect (mig 096, last reproduced
-- in mig 110), all backward-compatible (every new field is optional, defaults
-- preserve current behaviour):
--
--   * to: 'graveyard'   — tutor a card straight into the graveyard (Entomb,
--                         Buried Alive). The submit apply branch grows a graveyard
--                         destination alongside hand/battlefield/top.
--   * tapped: true      — a tutored permanent enters the battlefield TAPPED
--                         (Rampant Growth, fetchlands). Carried in params; the
--                         battlefield branch reads it (default false = untapped).
--   * filter.name       — match library cards by NAME substring, in addition to
--                         the existing filter.type_line (search for a card named X).
--   * reveal: true      — record the found card ids in the decision result under
--                         `revealed`. NOTE: this engine has no hidden-info /
--                         opponent-reveal modeling, so reveal is auditable METADATA
--                         only (no UI broadcast) — the honest minimal sink.
--
-- Reproduces apply_trigger_effects + submit_decision from mig 110 verbatim, with
-- surgical changes confined to the two search_library branches.
-- (IDE T-SQL false-positives on $$ bodies — ignore.)

-- ---------------------------------------------------------------------------
-- apply_trigger_effects (mig 110) — the search_library park branch now also
-- filters by name and stamps tapped/reveal into params. Everything else verbatim.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- submit_decision (mig 110) — the search_library apply branch grows a graveyard
-- destination, honours params.tapped for the battlefield case, and stamps
-- params.reveal into result.revealed. Everything else verbatim.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_decision.decision_type = 'scry' then
    ...
    where g.id = s.id;
    -- Reveal: record the found card ids as auditable metadata (no opponent-facing
    -- UI broadcast exists in this engine). Refresh v_decision so the row returned
    -- to the caller carries result.revealed.
