-- Undying (Geralf's Mindcrusher) — "When this creature dies, if it had no
-- +1/+1 counters on it, return it to the battlefield under its owner's control
-- with a +1/+1 counter on it." Top-level script flag `undying: true`, handled
-- entirely in put_in_graveyard (the battlefield->graveyard chokepoint):
--   capture plus_one_counters BEFORE the move (the move resets them);
--   complete the normal graveyard move (dies triggers + the death tally fire);
--   then return the card with one +1/+1 counter (fires ETB triggers again —
--   rules-correct for undying). Having a counter now, a second death is final.
-- Board-wipe loops iterate a pre-snapshotted dying set, so the mid-loop return
-- cannot re-enter the loop.
-- Generated from supabase/functions_src (put_in_graveyard) — those files are
