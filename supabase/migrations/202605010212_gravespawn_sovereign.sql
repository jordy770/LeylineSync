-- Gravespawn Sovereign — "Tap five untapped Zombies you control: Put target
-- creature card from a graveyard onto the battlefield under your control."
--   New activation cost { "type": "tap_creatures", "count": 5, "type_line":
--     "Zombie" } — validates you control that many untapped matching creatures
--     and taps the first N (zone order; the source may be among them; a
--     client-chosen set is a future refinement).
--   New targeted graveyard effect `reanimate_from_graveyard` — the mig 186
--     exile_from_graveyard pattern: direct-insert stack item + registered
--     handler + action_type CHECK extension. The handler puts the card onto
--     the battlefield under the ACTIVATOR's control (fires ETB triggers).
...
alter table public.game_stack_items
  ...
  check (action_type = any (array[
    ...
    'exile_from_graveyard', 'reanimate_from_graveyard'
  ]));
...
revoke all on function public.handle_reanimate_from_graveyard(uuid, public.game_stack_items) from public;

insert into public.stack_action_handlers (action_type, handler_fn, description) values
  ('reanimate_from_graveyard', 'handle_reanimate_from_graveyard', 'Put a target creature card from a graveyard onto the battlefield under the controller''s control')
...
begin
  ...
  elsif v_eff_type in ('create_token', 'search_library', 'grant_keyword_all') then
