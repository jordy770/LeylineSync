-- F3 — Protection (DEBT), slice 3: the "can't be Blocked" (B) gate.
--
-- A creature with protection from a colour can't be blocked by creatures of that
-- colour (CR 509.1b / 702.16e). When a blocker is declared, reject it if the
-- ATTACKER has protection from the BLOCKER's colour. (Protection imposes no
-- restriction on the protected creature itself blocking — only the "can't be
-- blocked by" direction, mirroring how the D gate still lets a protected creature
-- deal its own damage.)
--
-- declare_blocker is only defined in the baseline (no later migration redefines
-- it), so this reproduces the baseline body verbatim and adds one protection check
-- after the existing flying-legality gate, reusing game_card_color_set (mig 132)
-- and card_has_protection_from_any (mig 131). This completes the functional
-- protection gates (T/D/B); the E gate (auras/equipment) stays deferred.
