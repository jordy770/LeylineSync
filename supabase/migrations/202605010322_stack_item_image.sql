-- 202605010322_stack_item_image
-- Adds source_card_image_url + source_card_type_line to get_stack_items so the
-- controller can show a card thumbnail / zoom for each spell on the stack.
-- Adding columns changes the RETURNS TABLE shape, so the old function must be
-- dropped before the create-or-replace.
-- Generated from supabase/functions_src (get_stack_items) — those files are
-- the canonical current definitions; edit them, not past migrations.

drop function if exists public.get_stack_items(uuid);
