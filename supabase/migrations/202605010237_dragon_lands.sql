-- 202605010237_dragon_lands — Path of Ancestry, Temple of the Dragon Queen, Haven.
--   • cast_card_from_hand: enters_tapped `unless` conditions are now INDEPENDENT
--     (OR), so a land can untap on any of control_type / hand_has_type / count
--     (Temple: "unless you revealed a Dragon OR you control a Dragon").
--   • activate_ability routes a `return_from_graveyard` activated effect through
--     the spell_effect resolver (so it parks the pick-from-graveyard decision) —
--     Haven's "{2}, {T}, Sacrifice: return a Dragon from your graveyard."
-- Approximations (flagged): Path/Haven "any color" drops the colour-identity /
-- Dragon-only restriction; Path's scry-on-spend rider is deferred. Generated from
-- supabase/functions_src (cast_card_from_hand, activate_ability).
