-- 202605010361_xantcha
-- Xantcha, Sleeper Agent: new donate_self effect (source enters under an opponent's
-- control). activate_ability now keys the source lookup on CONTROL, not ownership,
-- so a donated/stolen permanent's abilities work for its controller. APPROX:
-- forced-attack + any-player-activate not modelled.
-- Generated from supabase/functions_src (apply_triggered_ability_effects, activate_ability) — those files are
