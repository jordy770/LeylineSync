-- 202605010260_dino_combat
-- Veloci-Ramp-Tor combat batch (4 cards): Quartzwood Crasher, Wrathful
-- Raptors, From the Rubble (script-only), Itzquinth (script-only).
--   • fire_watcher_triggers gains p_extra (event context → enqueue payload);
--     the old 4-arg overload is dropped below.
--   • apply_damage_to_creature broadcasts 'creature_damaged' watchers with
--     event_amount (Wrathful Raptors: the damaged Dino redirects that much
--     damage; the "isn't a Dinosaur" target restriction is NOT enforced).
--   • resolve_combat_damage tallies player damage from TRAMPLE creatures and
--     broadcasts 'trample_combat_damage' (batched per damaged player).
--   • create_token set_pt / set_pt:'event_amount' (Quartzwood: X/X token via
--     an unexpiring set_pt row; zero damage → no token).
--   • amount:'event_amount' substitution in apply_trigger_effects.
--   • From the Rubble: choose_creature_type ($chosen bake) + end-step
--     return_from_graveyard to battlefield. The finality counter is NOT
--     modelled (the returned creature dies normally).
--   • Itzquinth: ETB targeted damage = greatest Dinosaur power you control.
--     Approximations: the {2} payment is skipped (always free), and the
--     dealer is implicitly your strongest Dinosaur.
-- Generated from supabase/functions_src (apply_damage_to_creature, apply_trigger_effects, apply_triggered_ability_effects, fire_watcher_triggers, resolve_combat_damage) — those files are
-- the canonical current definitions; edit them, not past migrations.

-- fire_watcher_triggers gains p_extra: drop the old 4-arg overload so the
-- 5-arg (defaulted) version is the only resolution candidate.
drop function if exists public.fire_watcher_triggers(uuid, uuid, uuid, text);
