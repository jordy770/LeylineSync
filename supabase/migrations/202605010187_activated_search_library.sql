-- search_library as an ACTIVATED-ability effect (Wayfarer's Bauble: "{1}, {T},
-- Sacrifice Wayfarer's Bauble: Search your library for a basic land card, put it
-- onto the battlefield tapped, then shuffle"). The tutor parks a resolution-time
-- decision, so — exactly like create_token — it routes through a spell_effect
-- stack item (the spell-effect resolver runs search_library, incl. the `tapped`
-- flag and the `filter`). The sacrifice_self cost (mig 175) is paid first.
--
-- Reproduced from migration 186; the ONLY change is the single-effect routing
-- branch now fires for search_library as well as create_token.
...
begin
  ...
  elsif v_eff_type in ('create_token', 'search_library') then
    -- A single create_token / search_library effect (the tutor parks a decision)
