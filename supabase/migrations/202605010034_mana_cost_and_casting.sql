...
set script = jsonb_build_object(
  ...
  'triggers',
  jsonb_build_array('manual_tap')
)
where lower(name) = 'forest'
  and (script is null or script = '{}'::jsonb);
...
begin
  ...
  if p_source_card_id is not null and v_source_zone = 'hand' then
    perform public.pay_mana_cost(p_session_id, auth.uid(), v_source_mana_cost);
  end if;
...
grant execute on function public.pay_mana_cost(uuid, uuid, text) to authenticated;
