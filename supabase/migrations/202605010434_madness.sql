-- 202605010434_madness
-- Madness (From Under the Floorboards, Markov Baron): the new discard_card
-- helper intercepts every discard path — a HAND card whose script carries
-- `madness` is discarded into EXILE with a stack-less madness_cast pending
-- decision for its owner; all other cards take the normal hand→graveyard
-- move. The bulk/random discard branches in apply_trigger_effects and the
-- choose_cards→graveyard picks in submit_decision now route through the
-- helper. submit_decision gains a madness_cast branch: {cast:true, x?} pays
-- the madness cost and casts from exile (a permanent as a cast_permanent
-- push, a spell as its X-substituted madness_effect program, with spell_cast
-- watchers + cast triggers fired); {cast:false} puts it in the graveyard.
-- Generated from supabase/functions_src (discard_card, apply_trigger_effects, submit_decision) — those files are
