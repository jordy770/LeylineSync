-- Phase 1, slice 13 — multi-opponent edict: sacrifice who:'each_opponent'.
--
-- "Each opponent sacrifices a creature" (Edict of Cruelty / Plaguecrafter). Each
-- opponent chooses their OWN creature, so it's a CHAIN of sequential decisions —
-- one per opponent — that must all finish before the spell/ability resumes.
--
-- Mechanism (no new decision_type, reuses 'sacrifice'): the parked decision now
-- carries in its params the remaining-opponent QUEUE plus the count/filter. When a
-- sacrifice resolves (submit_decision), if the queue is non-empty we park the NEXT
-- opponent instead of resuming; only when the queue empties do we resume the
-- program. The stack item's resume_index is set ONCE (in apply_trigger_effects)
-- and untouched across the chain, so the program continues correctly at the end.
--
-- The build-options-and-park step is shared by the initial park and every
-- next-in-queue park via the new park_edict_sacrifice helper — so the existing
-- who:'you' (queue=[controller]) and who:'opponent' (queue=[first opponent]) cases
-- route through it too, with an empty leftover queue (one decision, then resume).
-- This keeps SAC1/2/3 behaviour-identical. The helper skips a candidate with no
-- eligible creature and tries the next (an opponent with an empty board is simply
-- not prompted). Reproduces apply_trigger_effects + submit_decision from mig 108
-- with surgical changes. (IDE T-SQL false-positives on $$ bodies — ignore.)

-- ---------------------------------------------------------------------------
-- Helper: park a sacrifice decision for the first candidate in p_queue (an
-- ordered jsonb array of player-id strings) that controls an eligible permanent.
-- Stores {count, filter, edict_queue: <candidates after the chosen one>} so the
-- chain can continue. Returns the decision id, or null if NO candidate can
-- sacrifice. SECURITY DEFINER, reached only via the two definer functions below.
-- ---------------------------------------------------------------------------
create or replace function public.park_edict_sacrifice(
  p_session_id uuid,
...
declare
  ...
  v_decision_id uuid;
begin
  v_n := jsonb_array_length(coalesce(p_queue, '[]'::jsonb));

  while v_idx < v_n loop
    v_decider := (p_queue ->> v_idx)::uuid;
    ...
    where gc.session_id = p_session_id and gc.controller_player_id = v_decider and gc.zone = 'battlefield'
      and (case when p_filter is not null then c.type_line ilike '%' || p_filter || '%'
                else c.type_line ilike '%creature%' end);

    v_len := jsonb_array_length(v_options);
    if v_len > 0 then
      v_amount := least(coalesce(p_count, 1), v_len);

      -- Candidates after the current one (0-based idx > v_idx; ordinality is 1-based).
      select coalesce(jsonb_agg(e order by ord), '[]'::jsonb) into v_remaining
      from jsonb_array_elements(p_queue) with ordinality as t(e, ord)
      where ord > v_idx + 1;
      ...
      values (p_session_id, v_decider, p_stack_item_id, 'sacrifice', 'Sacrifice ' || v_amount, v_options, v_amount, v_amount,
        jsonb_build_object('count', coalesce(p_count, 1), 'filter', p_filter, 'edict_queue', v_remaining))
...
revoke all on function public.park_edict_sacrifice(uuid, uuid, integer, text, jsonb) from public;

-- ---------------------------------------------------------------------------
-- apply_trigger_effects (mig 108) — the sacrifice branch now builds a decider
-- QUEUE by `who` and parks via the helper. Everything else is verbatim.
-- ---------------------------------------------------------------------------
...
begin
  ...
  while v_i < v_count loop
    ...
    if v_type in ('scry', 'surveil') then
      ...
      -- Build the ordered candidate queue by `who`, then park the first candidate
      -- that can actually sacrifice (the helper skips empty boards). who:'you' =
      -- the controller; 'opponent' = the first opponent (an edict); 'each_opponent'
      -- = every opponent in seat order, each prompted in turn (the chain).
      ...
      if v_decision_id is null then v_i := v_i + 1; continue; end if;  -- nobody can sacrifice
...
-- ---------------------------------------------------------------------------
-- submit_decision (mig 108) — the sacrifice apply branch now continues the edict
-- chain: park the next opponent if the queue is non-empty, else resume. The rest
-- is verbatim.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_decision.decision_type = 'scry' then
    ...
    -- Edict chain: if opponents remain in the queue, park the next one (skipping
    -- empty boards) instead of resuming. Only when none remain do we resume the
    -- program. For who:'you'/'opponent' the queue is empty → resume immediately.
    v_decision_id := public.park_edict_sacrifice(
