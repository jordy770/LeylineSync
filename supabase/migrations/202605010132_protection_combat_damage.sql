-- F3 — Protection (DEBT), slice 2: the "can't be Damaged" (D) gate for COMBAT.
--
-- Single-target spell/ability/trigger damage is already covered by slice 1's T
-- gate (a protected creature can't be chosen as the target, so it never takes that
-- damage). The uncovered case is COMBAT damage, which is dealt without targeting:
-- an attacker and its blockers damage each other regardless of protection. Here a
-- creature with protection from the DAMAGING creature's colour takes no combat
-- damage from it (CR 702.16e / 510.4 — the damage is still ASSIGNED, so trample
-- math is unchanged, but it isn't dealt).
--
-- Reproduces resolve_combat_damage from its CURRENT definition (mig 127) verbatim
-- except the two creature-damage sites, which now skip the damage_marked update (and
-- the creature-damage tally) when the victim has protection from the dealer's
-- colour. Player damage is untouched (players have no colour protection). A new
-- helper game_card_color_set(session, game_card) gives a battlefield card's colours
-- from its mana cost (card_color_set, mig 131). (IDE T-SQL false-positives on $$.)

-- ---------------------------------------------------------------------------
-- A game card's colour set, from its catalog mana cost. Null when the card isn't
-- in the session (card_has_protection_from_any treats null as "no colours").
-- ---------------------------------------------------------------------------
create or replace function public.game_card_color_set(p_session_id uuid, p_game_card_id uuid)
returns text[]
language sql
stable
security definer
...
as $$
  select public.card_color_set(c.mana_cost)
...
grant execute on function public.game_card_color_set(uuid, uuid) to anon, authenticated, service_role;
...
declare
  ...
  -- Player-chosen assignment state
  v_chosen jsonb;
  v_trample_amount integer;
  -- Validation pre-pass state
  v_va_key text;
  v_va_chosen jsonb;
...
begin
  ...
  -- ── Validation pre-pass: reject illegal player-chosen distributions before any
  -- damage is written. Skips attackers not in this turn's unresolved combat.
  ...
  loop
    ...
    -- This attacker's player-chosen distribution (null = auto-assign).
    v_chosen := case
    ...
    if not v_has_blockers then
      if v_attacker_deals_damage and v_attacker_damage > 0 then
        -- Unblocked: route through the prevention resolver (combat damage).
      ...
      loop
        ...
        if v_attacker_deals_damage and v_remaining_attacker_damage > 0 then
          if v_chosen is not null then
            -- Player-chosen amount for this blocker (validated above); 0 if omitted.
            ...
            -- Auto: deathtouch makes 1 lethal, so only 1 needs assigning per blocker.
          ...
          if v_assigned_damage > 0 then
            -- Protection: damage is still ASSIGNED (so the remainder/trample math is
            -- unchanged) but only DEALT if the blocker has no protection from the
            -- attacker's colour.
        ...
        if v_blocker_deals_damage and v_blocker_damage > 0 then
          -- Protection: the attacker takes the blocker's damage only if it has no
          -- protection from the blocker's colour.
          if not public.card_has_protection_from_any(
      ...
      then
        -- Auto: trample the entire remainder. Chosen: trample the requested amount
        -- (validated to be <= power and legal only when all blockers are lethal),
        -- capped at what is actually left.
        if v_chosen is not null then
        ...
        if v_trample_amount > 0 then
          -- Trample spillover to the player: also through the prevention resolver.
          v_total_player_damage := v_total_player_damage + public.apply_damage_to_player(
