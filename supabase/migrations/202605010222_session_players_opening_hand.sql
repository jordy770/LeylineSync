-- get_session_players gains mulligans + opening_hand_kept (mig 221 columns) so
-- the controller's opening-hand overlay can render keep/mulligan state for
-- every player. Return-type change => drop the old signature first.

drop function if exists public.get_session_players(uuid);
-- Generated from supabase/functions_src (get_session_players) — those files are
