-- Dynamic / negatable pump + pump as a targetable effect (for Liliana, Untouched by
-- Death's −2: "Target creature gets -X/-X until end of turn, where X is the number of
-- Zombies you control"). Two changes:
--   1. trigger_effect_target_type (reproduced from mig 130) adds `pump` to the
--      creature-targetable effects, so a loyalty/trigger pump routes through the
--      targeted path (enqueue_triggered_ability requires + the player picks a target).
--   2. apply_creature_effect (reproduced from mig 162) — the pump branch resolves
--      `power`/`toughness` via resolve_dynamic_amount (so they can be a {count:…}
--      object) and negates each when the value object has `"negate": true`; a debuff
--      to ≤0 toughness runs the lethal SBA. Fixed-number pumps are unchanged.
