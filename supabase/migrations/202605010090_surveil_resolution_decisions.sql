...
--
-- Surveil N: look at the top N of your library, put any number into your
-- graveyard, the rest back on top in any order. Same park/resume shape as scry
-- (089): cast_surveil pushes a 'surveil' item; resolve_top_of_stack parks it and
-- opens a 'surveil' decision; submit_decision bins the chosen cards, reorders the
-- rest, then finalize_stack_resolution resumes.
--
-- submit_decision result for surveil: { "graveyard": [game_card_id, ...],
-- "top": [game_card_id, ...] } (graveyard ordered first-binned-first; top ordered
-- top-to-bottom). Together they must be exactly the revealed cards.
-- Library convention: top = LOWEST zone_position. (IDE T-SQL false-positives on $$.)

-- Allow the 'surveil' stack action type (full list = 089 list + 'surveil').
...
alter table public.game_stack_items
  ...
  check (action_type = any (array[
    ...
    'exile_creature', 'modal_spell', 'scry', 'surveil'
  ]));

-- Announce a surveil. Pushes a 'surveil' stack item; the decision is created at
-- resolution time.
create or replace function public.cast_surveil(
  p_session_id uuid,
...
begin
  ...
  if coalesce(p_amount, 0) < 1 then
    raise exception 'Surveil amount must be at least 1';
  end if;
...
grant execute on function public.cast_surveil(uuid, integer, uuid) to authenticated;

-- submit_decision: reproduced from 089 with a 'surveil' branch added.
...
begin
  ...
  if v_decision.decision_type = 'scry' then
    ...
    -- Bin the chosen cards into the graveyard (in the submitted order).
    with g as (
    ...
    -- Reorder what stays: kept "top" first (in order), then un-revealed library.
    with ordered as (
      select (t.value)::uuid as id, 0 as section, t.ord as ordnum
      from jsonb_array_elements_text(v_top) with ordinality as t(value, ord)
      union all
...
-- resolve_top_of_stack: reproduced from 089 with a 'surveil' branch (parks the
-- item like scry). Everything else unchanged.
