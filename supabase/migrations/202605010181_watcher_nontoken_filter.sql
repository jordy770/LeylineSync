-- `nontoken` watcher filter — "Whenever a NONTOKEN creature you control dies, …"
-- (Midnight Reaper, Open the Graves, Liliana's Standard Bearer). The watcher only
-- fires when the entering/dying creature is not a token.
--
-- Reproduced verbatim from migration 165 (fire_watcher_triggers) with one
-- addition: fetch the changed card's is_token and, when filter.nontoken is true,
-- skip token creatures.
