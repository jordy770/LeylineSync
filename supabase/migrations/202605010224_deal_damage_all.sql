-- Mass damage `deal_damage_all` (Blasphemous Act, Storm's Wrath, Harbinger of
-- the Hunt). Untargeted: N damage to every battlefield creature matching an
-- optional filter, and optionally to planeswalkers (targets:
-- 'creatures_planeswalkers'). filter.with_keyword / without_keyword gate on
-- flying (Harbinger's two halves), filter.exclude_source skips the source
-- ("each OTHER creature"). Routes damage through apply_damage_to_creature
-- (shields/deathtouch-aware, sweep deferred) + apply_damage_to_planeswalker,
-- then one lethal + zero-loyalty sweep. Works as a spell, trigger, or single
-- activated ability (activate_ability routes it via the spell_effect insert).
-- NOTE: Blasphemous Act's cost reduction isn't modelled (it costs full price).
...
begin
  ...
  elsif v_eff_type in ('create_token', 'search_library', 'grant_keyword_all', 'return_all_from_graveyard', 'deal_damage_all') then
