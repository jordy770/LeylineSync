-- 202605010433_evoke_blitz_spectacle
-- Three one-card alternative-cost mechanics:
-- - Evoke (Mulldrifter): cast_card_from_hand gains p_alt_cost ('evoke' |
--   'blitz', a whitelisted script key whose mana string replaces the printed
--   cost, stamped as a counter marker). handle_cast_permanent sacrifices an
--   evoked permanent right after its ETB triggers are enqueued.
-- - Blitz (Mayhem Patrol): same p_alt_cost path; on resolution the permanent
--   gains haste + a granted dies-draw and is marked with the existing
--   cleanup_at_end_step sweep (advance_step → put_in_graveyard, so the
--   dies-draw fires on the end-step sacrifice).
-- - Spectacle (Light Up the Stage): cast_spell_effect gains p_spectacle —
--   pays the script's `spectacle` cost instead of the printed cost, only
--   while an opponent has lost life this turn (life_lost_this_turn tracker).
-- Generated from supabase/functions_src (cast_card_from_hand, cast_spell_effect, handle_cast_permanent) — those files are
...
-- old ones. Drop the old signatures first (same pattern as mig 418/428-432).
drop function if exists public.cast_card_from_hand(uuid, uuid, jsonb, uuid, boolean, uuid[], integer, uuid[]);
drop function if exists public.cast_spell_effect(uuid, jsonb, uuid, integer, uuid, boolean, boolean, boolean, boolean, uuid[], uuid[]);
