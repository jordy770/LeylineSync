-- 202605010232_monstrosity — Stormbreath Dragon's monstrosity.
--   • `monstrosity` effect (activated): become monstrous once — N +1/+1 counters
--     + a 'monstrous' marker in the counter bag — then apply its `on_monstrous`
--     effects. A no-op once monstrous. Routed through a spell_effect stack item
--     by activate_ability (so it reuses the spell resolver).
--   • `damage_each_opponent_by_hand`: Stormbreath's become-monstrous rider —
--     each opponent takes damage equal to the cards in their OWN hand (per-player,
--     so it can't use the single-amount lose_life path).
-- No DDL — the 'monstrous' marker is a counter-bag entry.
-- Generated from supabase/functions_src (activate_ability, apply_triggered_ability_effects).
