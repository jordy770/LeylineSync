-- 202605010356_saw_in_half
-- Saw in Half: new saw_in_half targeted effect (grant a copy_self ×2 dies-trigger
-- with half P/T baked from current effective P/T, then destroy). New copy_self
-- effect (N token copies of the source, used from the grave on death).
-- Generated from supabase/functions_src (apply_creature_effect, apply_triggered_ability_effects, trigger_effect_target_type) — those files are
  ...
  select case
    when lower(coalesce(p_effect ->> 'type', '')) in
         ('deal_damage', 'destroy', 'exile', 'bounce', 'tap', 'untap',
          ...
          'exile_and_manifest', 'ignition', 'exile_until_leaves', 'blink', 'saw_in_half')
