...
-- creature token." (card request).
--
-- Two new pieces of engine vocabulary:
--   1. A multi-action TARGETED spell program needs to carry its target. cast_spell_effect
--      gains p_target_card_id, stored on the spell_effect stack item as target_card_id
--      (apply_trigger_effects already reads payload->>'target_card_id'). This is the
--      general fix for "destroy target permanent + <rider>" spells, not Beast-specific.
--   2. create_token with recipient:"target_controller" — the token is created under the
--      control of the SPELL'S TARGET's controller (usually an opponent), NOT the caster.
--      The catch: the controller must be captured BEFORE the destroy moves the permanent
--      to the graveyard. apply_trigger_effects captures the target's live controller once
--      before its effect loop and injects it as recipient_player_id into any create_token
--      with recipient:"target_controller"; apply_triggered_ability_effects then creates
--      the token (owner AND controller) for that player.
--
-- Reproduced from CURRENT (grep-first): cast_spell_effect (109), apply_trigger_effects
-- (154), apply_triggered_ability_effects (161). DROP the 4-arg cast_spell_effect first
-- (adding a defaulted 5th param to it would make 4-arg calls ambiguous — bug-421).
-- (IDE T-SQL false-positives on $$ bodies — ignore.)

-- ===========================================================================
-- cast_spell_effect (CURRENT = mig 109) + p_target_card_id → stored on the stack item
-- so apply_trigger_effects can resolve a targeted action in the program.
-- ===========================================================================
drop function if exists public.cast_spell_effect(uuid, jsonb, uuid, integer);
...
begin
  ...
  -- Non-permanent spell cast from hand: pay its mana cost (including {X}). No-op when
  -- the source is sourceless or has no mana_cost (the free-cast test fixtures).
...
-- ===========================================================================
-- apply_trigger_effects (CURRENT = mig 154) — capture the target's LIVE controller
-- before the effect loop (the destroy will move it to the graveyard), and inject it
-- as recipient_player_id into any create_token with recipient:"target_controller".
-- Everything else verbatim from 154.
-- ===========================================================================
...
-- ===========================================================================
-- apply_triggered_ability_effects (CURRENT = mig 161) — create_token honors an
-- injected recipient_player_id (owner AND controller of the token) so "its controller
-- creates a token" gives the token to the destroyed permanent's controller, not the
-- caster. Only the create_token branch changes; the rest is verbatim from 161.
-- ===========================================================================
...
begin
  ...
  loop
    ...
    if v_eff_type = 'gain_life' then
      ...
      -- Beast Within: a recipient_player_id (the targets controller, injected
      -- by apply_trigger_effects) overrides the caster as the tokens owner+controller.
