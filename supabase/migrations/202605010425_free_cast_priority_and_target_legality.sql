-- free_cast_priority_and_target_legality
-- p_free also bypasses the priority-player gate (off-turn cascade nested-cast); and
-- cast_card_free bottoms a targeted found spell with no legal target (sentinel) instead
-- of parking a dead item that fizzles to the graveyard (design fallback taxonomy).
