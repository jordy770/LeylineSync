-- Flashback with an additional "Pay N life" cost (Deep Analysis: "Flashback—
-- {1}{U}, Pay 3 life"). The flashback mana cost is the script's `flashback`
-- string; the life payment is a new optional `flashback_life` integer. Only the
-- FLASHBACK (graveyard) cast pays it — a normal hand cast is unaffected.
--
-- Reproduced verbatim from migration 174 with two additions:
--   * declare v_flashback_life
--   * in the graveyard/flashback branch, after paying the flashback mana, pay the
--     life (you can't pay life you don't have).
