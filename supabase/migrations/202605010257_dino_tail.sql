-- 202605010257_dino_tail — the Veloci-Ramp-Tor tail batch (~10 cards;
-- scripts in card-scripts.json).
--   • resolve_count_amount gains a SOURCE param (SIGNATURE CHANGE — old
--     3-arg overload dropped below): exclude_self counts ("each OTHER
--     Dinosaur", Earthshaker Dreadmaw) and a greatest_power_you_control
--     count with type inversion (Rishkar's Expertise; Return of the
--     Wildspeaker's NON-Human). resolve_dynamic_amount forwards its source.
--   • ignition targeted kind (Chandra's Ignition): the target deals its
--     power to each other creature and each opponent of the caster.
--   • choose_one joins the activated-ability routing (Shifting Ceratops'
--     {G}: reach/trample/haste pick).
-- Script-only: Lifecrafter's Bestiary (upkeep scry + pay-{G}-draw watcher),
-- Rhythm of the Wild (riot via a modal watcher with reflexive mode actions;
-- the can't-be-countered half is not modelled), Wayward Swordtooth
-- (additional land play + the blessing attack gate; can't-BLOCK half not
-- enforced), Fiery Confluence (choose three, repeats allowed), Sunfrill
-- Imitator (become_copy on attack; the name/ability exception is not
-- modelled), Rishkar's free cast approximated as a put-from-hand (permanents
-- only).
-- Generated from supabase/functions_src (resolve_count_amount, resolve_dynamic_amount, apply_creature_effect, trigger_effect_target_type, activate_ability) — those files are
-- the canonical current definitions; edit them, not past migrations.

-- The source param changes the signature; drop the old overload.
drop function if exists public.resolve_count_amount(uuid, uuid, jsonb);
