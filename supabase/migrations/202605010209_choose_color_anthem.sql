-- Heraldic Banner — "As this artifact enters, choose a color. Creatures you
-- control of the chosen color get +1/+0. {T}: Add one mana of the chosen
-- color." Three pieces:
--   `choose_color` ETB decision effect ({ type, anthem: {power,toughness,scope} })
--     parks a five-colour pick (apply_trigger_effects); submit_decision
--     registers a pump continuous effect with the chosen colour in its payload
--     (source = the banner, battlefield-gated, NOT script-flagged so rebuilds
--     keep the choice);
--   the mass-pump fold in card_layered_power/toughness gains a payload.color
--     filter (card_color_set over the creature's mana cost);
--   the mana ability is authored as color "any" (the client's any-colour picker)
--     — restricting the picker to the chosen colour is a client follow-up.
-- Generated from supabase/functions_src (card_layered_power, card_layered_toughness, apply_trigger_effects, submit_decision) — those files are
