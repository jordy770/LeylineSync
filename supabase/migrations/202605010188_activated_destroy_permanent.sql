-- Activated destroy/exile/bounce/tap/untap of a NON-creature permanent (Unstable
-- Obelisk: "{7}, {T}, Sacrifice Unstable Obelisk: Destroy target permanent.").
-- The creature-effect builder rejects non-creature targets, so when the effect's
-- target_type isn't creature-only the branch routes to the type-flexible
-- `permanent_effect` action (mig 113) instead of `<kind>_creature`. Creature-only
-- targets (the common case) are unchanged.
--
-- Reproduced from migration 187; the ONLY change is the removal branch.
