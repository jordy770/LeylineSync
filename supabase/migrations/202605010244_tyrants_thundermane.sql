-- 202605010244_tyrants_thundermane — Leyline Tyrant + Hammerhead Tyrant +
-- Thundermane Dragon.
--   • public.mana_value(text): mana value parsed from mana_cost (numeric
--     symbols add their value, X adds 0, anything else adds 1);
--     resolve_count_amount's greatest_mana_value branch now uses it.
--   • pay_x_mana_damage (Leyline Tyrant dies trigger): parks targets + a
--     colour; submit validates the amount against the payer's pool, deducts
--     it, and damages the pick. NOTE: the mana-retention half needed NO
--     engine change — clear_mana_pool_for_step already honours
--     mana_does_not_empty (the card is the first to script it).
--   • bounce_up_to / bounce_pick (Hammerhead Tyrant): parked bounce pick over
--     opponent nonland permanents, capped at the TRIGGERING cast spell's mana
--     value; picking nothing declines.
--   • cast_from_library_top continuous permission (Thundermane Dragon):
--     cast_card_from_hand accepts a LIBRARY source when it's the top card and
--     a permission's {creature, min_power} filter matches; grant_haste gives
--     the cast card haste until end of turn. "Look at the top card any time"
--     is a client concern (not modelled).
-- Generated from supabase/functions_src (mana_value, resolve_count_amount, register_card_continuous_effects, apply_trigger_effects, submit_decision, cast_card_from_hand) — those files are
-- the canonical current definitions; edit them, not past migrations.

-- cast_from_library_top joins the allowed continuous-effect types (latest
-- list mig 231).
