-- Phase 1, slice 9: more resolution-time choices on the shared decision system —
-- tutor (search_library), discard (choose_cards), and optional "may" (confirm).
--
-- All three are new effect types handled by apply_trigger_effects (so they work in
-- both triggered abilities and spell_effect programs): they build options, insert
-- a game_pending_decisions row with a new decision_type, and park. submit_decision
-- applies the outcome and resumes. Adds:
--   * game_pending_decisions.params jsonb  — per-decision config (destination, etc.)
--   * resume_or_finalize(session, item)    — shared "continue the program or finish"
--   * search_library / discard / may parking in apply_trigger_effects
--   * search_library / choose_cards / confirm handling in submit_decision
-- (IDE T-SQL diagnostics false-positive on $$ bodies — ignore.)

alter table public.game_pending_decisions
  add column if not exists params jsonb not null default '{}'::jsonb;

-- After a decision is applied, either resume the parked source (a triggered
-- ability or spell_effect program continues from resume_index, possibly parking
-- again) or, for a standalone item, just finalize it.
create or replace function public.resume_or_finalize(
  p_session_id uuid,
...
begin
  select action_type, coalesce((payload ->> 'resume_index')::integer, 0)
    into v_type, v_resume
  from public.game_stack_items
  where id = p_stack_item_id;

  if v_type in ('triggered_ability', 'spell_effect') then
    if public.apply_trigger_effects(p_session_id, p_stack_item_id, v_resume) is null then
      perform public.finalize_stack_resolution(p_session_id, p_stack_item_id);
    end if;
  else
    perform public.finalize_stack_resolution(p_session_id, p_stack_item_id);
  end if;
...
grant execute on function public.resume_or_finalize(uuid, uuid) to authenticated;

-- apply_trigger_effects: reproduced from 093 with park branches for search_library,
-- discard, and may added (alongside scry/surveil).
...
begin
  ...
  while v_i < v_count loop
    ...
    if v_type in ('scry', 'surveil') then
      ...
      values (
        p_session_id, v_controller, p_stack_item_id, v_type,
        initcap(v_type) || ' ' || v_amount, v_options, 0, jsonb_array_length(v_options)
      )
      returning id into v_decision_id;

      update public.game_stack_items
      set status = 'awaiting_decision', payload = payload || jsonb_build_object('resume_index', v_i + 1)
      where id = p_stack_item_id;

      return v_decision_id;

    elsif v_type = 'search_library' then
      v_count := jsonb_array_length(v_effects); -- (loop bound unchanged; recompute defensively)
      ...
      if jsonb_array_length(v_options) = 0 then
        v_i := v_i + 1;  -- nothing matches; fail to find
        continue;
      end if;
      ...
      values (
        p_session_id, v_controller, p_stack_item_id, 'search_library',
      ...
      select coalesce(
               jsonb_agg(jsonb_build_object('game_card_id', h.id, 'name', c.name) order by h.zone_position, h.id),
               '[]'::jsonb
             )
        into v_options
      from public.game_cards h
      ...
      if v_len = 0 then
        v_i := v_i + 1;  -- empty hand; nothing to discard
        continue;
      end if;

      -- Discard is mandatory up to N (or the whole hand if smaller).
      ...
      values (
        p_session_id, v_controller, p_stack_item_id, 'choose_cards',
        'Discard ' || v_amount, v_options, v_amount, v_amount,
        jsonb_build_object('to', 'graveyard')
      )
      returning id into v_decision_id;
      ...
      values (
        p_session_id, v_controller, p_stack_item_id, 'confirm',
        coalesce(v_effect ->> 'prompt', 'You may'), '[]'::jsonb, 0, 0,
...
-- submit_decision: reproduced from 094 with search_library / choose_cards / confirm
-- handling added, and the post-apply resume routed through resume_or_finalize.
...
begin
  ...
  if v_decision.decision_type = 'choose_mode' then
    ...
    if v_needs_target then
      ...
      if not exists (
        ...
        where gc.id = v_target_card and gc.session_id = v_decision.session_id
    ...
    from (select (value)::uuid as id from jsonb_array_elements_text(v_top)
          union all select (value)::uuid from jsonb_array_elements_text(v_bottom)) q;
    ...
    select array_agg((value ->> 'game_card_id')::uuid) into v_option_ids from jsonb_array_elements(v_decision.options);
    select array_agg(id) into v_chosen_ids
    from (select (value)::uuid as id from jsonb_array_elements_text(v_grave)
          union all select (value)::uuid from jsonb_array_elements_text(v_top)) q;
    v_chosen_ids := coalesce(v_chosen_ids, array[]::uuid[]);
    if cardinality(v_chosen_ids) <> cardinality(v_option_ids) then
  ...
  elsif v_decision.decision_type in ('search_library', 'choose_cards') then
    ...
    if (select count(distinct e) from unnest(v_chosen_ids) e) <> cardinality(v_chosen_ids) then
      raise exception 'Chose a card more than once';
    end if;
    if exists (select 1 from unnest(v_chosen_ids) e where e <> all(v_option_ids)) then
      raise exception 'Chose a card that was not offered';
    end if;
  end if;
  ...
  if v_decision.decision_type = 'scry' then
    with ordered as (
      ...
      union all
      select gc.id, 1, gc.zone_position::bigint
      from public.game_cards gc
      where gc.session_id = v_decision.session_id and gc.owner_id = v_decision.deciding_player_id
        and gc.zone = 'library' and gc.id <> all(v_option_ids)
      union all
      select (b.value)::uuid, 2, b.ord
    ...
    base as (select coalesce(max(zone_position), -1) as m from public.game_cards
             where session_id = v_decision.session_id and owner_id = v_decision.deciding_player_id and zone = 'graveyard')
    update public.game_cards gc set zone = 'graveyard', zone_position = base.m + g.ord, is_tapped = false
    from g, base where gc.id = g.id;

    with ordered as (
      select (t.value)::uuid as id, 0 as section, t.ord as ordnum
      from jsonb_array_elements_text(v_top) with ordinality as t(value, ord)
      union all
      select lib.id, 1, lib.zone_position::bigint
      from public.game_cards lib
      where lib.session_id = v_decision.session_id and lib.owner_id = v_decision.deciding_player_id
        and lib.zone = 'library' and lib.id <> all(v_option_ids)
    ),
    renum as (select id, (row_number() over (order by section, ordnum) - 1) as np from ordered)
    update public.game_cards g set zone_position = renum.np from renum where g.id = renum.id;
    perform public.resume_or_finalize(v_decision.session_id, v_decision.source_stack_item_id);
    ...
    loop
      if v_dest = 'battlefield' then
        select coalesce(max(zone_position), -1) + 1 into v_pos from public.game_cards
        where session_id = v_decision.session_id and owner_id = v_decision.deciding_player_id and zone = 'battlefield';
        ...
        set zone = 'battlefield', zone_position = v_pos, controller_player_id = owner_id,
            is_tapped = false, damage_marked = 0, entered_battlefield_turn_number = coalesce(v_turn, 0)
        where id = v_card;
      elsif v_dest = 'top' then
        select coalesce(min(zone_position), 0) - 1 into v_pos from public.game_cards
        where session_id = v_decision.session_id and owner_id = v_decision.deciding_player_id and zone = 'library';
        update public.game_cards set zone = 'library', zone_position = v_pos where id = v_card;
      else
        select coalesce(max(zone_position), -1) + 1 into v_pos from public.game_cards
        where session_id = v_decision.session_id and owner_id = v_decision.deciding_player_id and zone = 'hand';
        update public.game_cards set zone = 'hand', zone_position = v_pos, is_tapped = false where id = v_card;
      end if;
    end loop;

    -- Searching a library shuffles it afterward.
    update public.game_cards g
    set zone_position = s.rn
    from (
      select id, (row_number() over (order by random(), id) - 1) as rn
      from public.game_cards
      where session_id = v_decision.session_id and owner_id = v_decision.deciding_player_id and zone = 'library'
    ) s
    where g.id = s.id;
    ...
    loop
      select coalesce(max(zone_position), -1) + 1 into v_pos from public.game_cards
      where session_id = v_decision.session_id and owner_id = v_decision.deciding_player_id and zone = v_dest;
      update public.game_cards
      set zone = v_dest, zone_position = v_pos, is_tapped = false, damage_marked = 0
      where id = v_card;
    end loop;
    perform public.resume_or_finalize(v_decision.session_id, v_decision.source_stack_item_id);
    ...
    if coalesce((v_decision.result ->> 'confirmed')::boolean, false) then
      ...
      perform public.apply_targeted_triggered_ability_effects(
        ...
        coalesce(v_ctrl, v_decision.deciding_player_id),
        v_src_card,
        coalesce(v_decision.params -> 'effects', '[]'::jsonb),
        v_tgt
      );
    end if;
    perform public.resume_or_finalize(v_decision.session_id, v_decision.source_stack_item_id);
