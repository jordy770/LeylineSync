-- Choose a creature type (roadmap Tribal #6). "Choose a creature type, then <effect>"
-- (Distant Melody — "draw a card for each creature of that type you control"; the
-- chosen type also drives Gravespawn Sovereign / Vela style tribal choices).
--
-- Mirrors the choose_player decision flow: apply_trigger_effects parks a
-- 'choose_creature_type' decision (a curated type list) carrying the sub-effects;
-- submit_decision validates the pick and applies the sub-effects with the chosen type
-- INJECTED into any count-based amount's type_line (so "{count: creatures_you_control}"
-- becomes a count of that subtype). Reproduced from CURRENT (grep-first):
-- apply_trigger_effects (163), submit_decision (154). (IDE T-SQL false-positives.)
...
begin
  ...
  if v_decision.decision_type = 'scry' then
    ...
    -- Each chosen permanent: +1 to its +1/+1 (only if it already had one) and +1
    -- to every bag kind it has. Each chosen player: +1 to every bag kind.
    ...
    -- "{count: creatures_you_control}" → counts that subtype).
    select coalesce(jsonb_agg(
      case when jsonb_typeof(e.value -> 'amount') = 'object' and (e.value -> 'amount') ? 'count'
