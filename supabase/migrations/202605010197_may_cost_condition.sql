-- Liliana's Devotee — "…you may pay {1}{B}. If you do, create a Zombie." Adds an
-- optional CONDITION gate and an optional mana COST to the `may` effect: the may is
-- offered only when the condition holds, and on confirm the cost is paid before the
-- inner effects run. Reusable "you may pay X" primitive.
--
-- Reproduces apply_trigger_effects + submit_decision (mig 191) by exact line range.
