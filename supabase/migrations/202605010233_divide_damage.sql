-- 202605010233_divide_damage — divided damage from triggers/abilities.
--   • divide_damage_options(session, controller, filter): legal targets for the
--     decision (creatures/planeswalkers/players per {controller, types}).
--   • apply_damage_allocations(session, allocations): applies a resolved split
--     (creatures via apply_creature_effect, planeswalkers via the PW helper,
--     players lose life) + one zero-loyalty sweep + game-end check.
--   • apply_trigger_effects parks a `divide_damage` decision (Dragonlord Atarka
--     ETB: "5 damage divided among opponents' creatures/planeswalkers").
--   • submit_decision validates the allocation (sum = amount, targets offered,
--     count within max_targets) and applies it.
--   • activate_ability routes a `divide_damage` activated effect through the
--     spell_effect resolver (so it parks the same decision — Skarrgan Hellkite)
--     and honours an "Activate only if …" {counters, of, at_least} `condition`.
-- No new decision_type constraint (decision_type is unconstrained). Generated
-- from supabase/functions_src (divide_damage_options, apply_damage_allocations,
-- apply_trigger_effects, submit_decision, activate_ability).
