-- "Exile a creature card from a graveyard" as an activated-ability cost, plus
-- create_token as an activated-ability effect. Cemetery Reaper: "{2}{B}, {T},
-- Exile a creature card from a graveyard: Create a 2/2 black Zombie creature
-- token."
--
-- Reproduced verbatim from migration 175 with three additions:
--   * declare v_has_gy_exile / v_gy_filter / v_next_position
--   * an 'exile_from_graveyard' branch in the cost loop + a cost-payment block that
--     exiles the chosen graveyard card (passed as p_target_card_id; the effect is
--     untargeted). "From a graveyard" = any graveyard in the session.
--   * a 'create_token' effect branch that routes the token through a spell_effect
--     stack item (activate_ability has no per-effect token builder; this reuses the
--     spell-effect resolver, incl. the `tapped` flag).
...
begin
  ...
  if v_eff_type = 'draw' then
    ...
    -- activate_ability has no per-effect token builder; route the token through a
    -- spell_effect stack item so it reuses the spell-effect resolver
    -- (apply_trigger_effects → create_token, incl. the `tapped` flag).
