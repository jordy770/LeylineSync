-- Curse of Disturbance — "Enchant player. Whenever enchanted player is attacked,
-- create a 2/2 black Zombie creature token. Each opponent attacking that player
-- does the same." Modeled without a cast-path refactor: the curse's ETB
-- choose_player picks the enchanted player and registers a 'curse_attacked'
-- continuous effect on them (affected_player_id), sourced from the curse;
-- declare_attacker reacts when that player is attacked, enqueuing a create_token
-- for the curse's controller AND the attacking player.
--
-- Reproduces apply_triggered_ability_effects (mig 192) + declare_attacker (mig 169)
-- by exact line range. (IDE T-SQL false-positives on $$ bodies — ignore.)
...
alter table public.game_continuous_effects
  ...
  check (effect_type = any (array[
    ...
    'intimidate', 'hexproof', 'curse_attacked'
  ]));

-- apply_triggered_ability_effects (mig 192) + a curse_attack_zombie effect that
-- registers the curse on the recipient player (the ETB-chosen enchanted player).
...
-- declare_attacker (mig 169) + the curse-attacked trigger (enqueue Zombies).
