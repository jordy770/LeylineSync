-- Gisa and Geralf — "When ~ enters, mill four cards. Once during each of your
-- turns, you may cast a Zombie creature spell from your graveyard." The mill is
-- the existing ETB effect; this migration adds the STATIC, once-per-turn
-- cast-from-graveyard permission:
--   register_card_continuous_effects accepts 'cast_from_graveyard' in
--     continuous_effects (affected defaults to controller; payload carries
--     { type_line, once_per_turn }) — registered rows are rebuild-swept, so the
--     permission lives exactly while the card is on the battlefield;
--   cast_card_from_hand's graveyard gate picks a matching permission row,
--     preferring unrestricted ones; a once_per_turn permission is skipped when
--     its source's counter-bag 'gy_cast_turn' equals the current turn, and
--     stamped on use (the bag survives rebuilds — loyalty_turn precedent).
-- Casting is active-player-only in this engine, so "during each of YOUR turns"
-- is already enforced upstream.
-- Generated from supabase/functions_src (register_card_continuous_effects, cast_card_from_hand) — those files are
