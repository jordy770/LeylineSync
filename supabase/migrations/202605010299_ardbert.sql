-- 202605010299_ardbert
-- Ardbert, Warrior of Darkness (mig 299). Two small additions:
--   • fire_watcher_triggers gains a spell_color filter ("whenever you cast a
--     WHITE/BLACK spell" — the cast spell's mana cost must contain the symbol).
--   • add_counters_all gains an optional type_line filter ("each LEGENDARY
--     creature you control"), so the +1/+1 lands only on legendaries.
-- The until-EOT keyword (vigilance/menace) rides the existing grant_keyword_all.
-- Generated from supabase/functions_src (fire_watcher_triggers, apply_triggered_ability_effects) — those files are
