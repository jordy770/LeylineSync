-- Multi-counter model (Tier 1 + poison loss).
--
-- The engine modeled exactly one counter: game_cards.plus_one_counters (an int on
-- the P/T hot path — folded into card_effective_power/toughness, layers, combat,
-- lethal SBA). Rewriting that to jsonb would be a high-risk migration for zero gain
-- on counters that DON'T touch P/T. So this is ADDITIVE:
--
--   * plus_one_counters stays exactly as-is (no engine-math change).
--   * a `counters jsonb` bag is added to game_cards (permanent counters: charge,
--     quest, etc.) and game_session_players (player counters: poison, energy,
--     experience, …). Anything that isn't +1/+1 lives in the bag.
--
-- Then:
--   * add_counters / add_counters_all gain a `counter_type` (absent/plus_one_one →
--     the fast column, unchanged; any other kind → the bag).
--   * a new add_player_counters effect puts player counters on a player (this is how
--     poison gets onto a player — proliferate only MULTIPLIES what's already there).
--   * proliferate now targets permanents AND players: any permanent with a +1/+1 or
--     a bag counter, and any player with a bag counter; each chosen entity gets +1 of
--     every kind it already has.
--   * state-based: a player with poison >= 10 loses (CR 104.3c / 704.5c).
--
-- Reproduced (project rule — reproduce from CURRENT, not stale baseline):
--   apply_creature_effect (147), apply_triggered_ability_effects (098),
--   apply_trigger_effects + submit_decision (153), maybe_finish_game_session (baseline).
-- (IDE T-SQL false-positives on $$ bodies — ignore.)

-- ===========================================================================
-- Storage: additive jsonb bags. plus_one_counters is untouched.
-- ===========================================================================
...
-- Add p_delta to one key of a counter bag, clamping at 0 and dropping zero entries
-- (a bag never carries a key whose value is 0).
create or replace function public.adjust_counter_bag(p_bag jsonb, p_key text, p_delta integer)
returns jsonb
language plpgsql
immutable
as $$
declare
  v_new integer;
begin
  v_new := greatest(0, coalesce((p_bag ->> p_key)::integer, 0) + coalesce(p_delta, 0));
  if v_new = 0 then
    return coalesce(p_bag, '{}'::jsonb) - p_key;
  end if;
  return coalesce(p_bag, '{}'::jsonb) || jsonb_build_object(p_key, v_new);
end;
$$;

-- "+1 of every kind already present" — the proliferate bump for one entity's bag.
create or replace function public.proliferate_bag(p_bag jsonb)
returns jsonb
language sql
immutable
as $$
  select coalesce(
           (select jsonb_object_agg(k, (v::integer) + 1) from jsonb_each_text(p_bag) as e(k, v)),
           '{}'::jsonb);
$$;

-- A counter_type that maps to the fast +1/+1 column rather than the bag.
create or replace function public.is_plus_one_counter(p_type text)
returns boolean
language sql
immutable
as $$
  select coalesce(lower(p_type), 'plus_one_one') in ('', 'plus_one_one', 'plus_one_plus_one', '+1/+1', 'p1p1');
$$;

-- ===========================================================================
-- apply_creature_effect (CURRENT = mig 147) — add_counters honors counter_type.
-- Only the add_counters branch changes; everything else is verbatim.
-- ===========================================================================
  ...
  if p_kind = 'deal_damage' then
    ...
    if v_amount > 0 then
      ...
      if public.is_plus_one_counter(v_counter_type) then
        ...
        update public.game_cards
        set counters = public.adjust_counter_bag(counters, lower(v_counter_type), v_amount)
        where id = p_target_card_id and session_id = p_session_id and zone = 'battlefield';
      end if;
    end if;
...
-- ===========================================================================
-- apply_triggered_ability_effects (CURRENT = mig 098) — counter_type on
-- add_counters / add_counters_all + new add_player_counters branch.
-- ===========================================================================
...
begin
  ...
  loop
    ...
    if v_eff_type = 'gain_life' then
      ...
      -- Put player counters (poison/energy/experience/…) on the chosen players.
      -- Default recipient each_opponent (poison/infect is dealt to opponents).
      ...
      if v_eff_amount > 0 then
        ...
        foreach v_rid in array coalesce(v_recipients, array[]::uuid[]) loop
          if v_rid is not null then
            update public.game_session_players
            set counters = public.adjust_counter_bag(counters, v_counter_type, v_eff_amount)
            where session_id = p_session_id and player_id = v_rid;
          end if;
        end loop;
      ...
      if p_source_card_id is not null and v_eff_amount <> 0 then
        ...
        if public.is_plus_one_counter(v_counter_type) then
          ...
          update public.game_cards
          set counters = public.adjust_counter_bag(counters, lower(v_counter_type), v_eff_amount)
          where id = p_source_card_id and session_id = p_session_id and zone = 'battlefield';
        end if;
      end if;
      ...
      if v_eff_amount > 0 and p_controller_id is not null then
        ...
        if public.is_plus_one_counter(v_counter_type) then
          ...
          update public.game_cards gc
          set counters = public.adjust_counter_bag(gc.counters, lower(v_counter_type), v_eff_amount)
          from public.cards c
          where c.id = gc.card_id and gc.session_id = p_session_id and gc.zone = 'battlefield'
            and c.type_line ilike '%creature%'
            and (
...
-- ===========================================================================
-- maybe_finish_game_session (CURRENT = baseline) — poison >= 10 also kills.
-- The "alive" predicate everywhere becomes: life_total > 0 AND poison < 10.
-- ===========================================================================
create or replace function public.maybe_finish_game_session(p_session_id uuid)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if v_session_status = 'finished' then
    return jsonb_build_object(
      'finished', true,
      'winner_player_id', (select winner_player_id from public.game_sessions where id = p_session_id)
    );
  end if;

  select
    count(*) filter (where life_total > 0 and coalesce((counters ->> 'poison')::integer, 0) < 10),
    count(*)
  ...
  if v_alive_count = 1 then
    ...
    where session_id = p_session_id
      and life_total > 0
      and coalesce((counters ->> 'poison')::integer, 0) < 10
    limit 1;

    update public.game_sessions
    set status = 'finished', finished_at = coalesce(finished_at, now()), winner_player_id = v_winner_player_id
    where id = p_session_id;

    return jsonb_build_object('finished', true, 'winner_player_id', v_winner_player_id);
  end if;

  if v_alive_count = 0 then
    update public.game_sessions
    set status = 'finished', finished_at = coalesce(finished_at, now()), winner_player_id = null
    where id = p_session_id;
...
-- ===========================================================================
-- apply_trigger_effects (CURRENT = mig 153) — proliferate now offers bag-bearing
-- permanents AND players with a counter. Only the proliferate park branch changes.
-- ===========================================================================
...
-- ===========================================================================
-- submit_decision (CURRENT = mig 153) — proliferate apply now bumps both the
-- +1/+1 column (if present) and every bag kind, for cards AND players.
-- Only the proliferate apply branch changes.
-- ===========================================================================
...
-- ===========================================================================
-- get_session_players — surface the player counter bag to the client (so poison
-- etc. are visible). Return-shape change → drop + recreate.
-- ===========================================================================
drop function if exists public.get_session_players(uuid);
create function public.get_session_players(p_session_id uuid)
returns table (
  session_id uuid,
  player_id uuid,
  username text,
...
as $$
  select
    ...
    coalesce(nullif(profiles.username, ''), left(game_session_players.player_id::text, 8)) as username,
    game_session_players.seat_number,
    game_session_players.life_total,
    game_session_players.counters,
    game_session_players.joined_at
  from public.game_session_players
  left join public.profiles on profiles.id = game_session_players.player_id
