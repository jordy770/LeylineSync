-- Phase 4 — activated abilities beyond deal_damage. "{cost}: effect" abilities can
-- now resolve the full effect vocabulary, not just damage: destroy / exile /
-- bounce / tap / untap / add_counters / pump / grant_keyword / gain_control of a
-- target creature, and untargeted draw. (Mana abilities still use their own flow.)
--
-- The engine was already 95% ready: put_action_on_stack's builders + the mig-104
-- resolve dispatcher handle every one of these action types for SPELLS; the only
-- barrier was activate_ability hardcoding `if effect.type = 'deal_damage'`. This
-- reproduces activate_ability (baseline) verbatim except that single branch, which
-- becomes a dispatch deriving the action_type + payload per effect and delegating
-- to put_action_on_stack — exactly as the deal_damage case already did.
--
-- The source is a battlefield permanent, so put_action_on_stack pays NO mana and
-- does NO graveyard move (those are hand-only) — the ability's own cost was already
-- paid above. Targets are validated by the builders (creature_target_controller_ok).
