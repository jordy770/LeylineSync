...
-- tapped. Flashback {7}{B}{B}{B}." Two contained additions:
--
--   1. Flashback — cast_spell_effect (reproduced from mig 163) now accepts a source
--      in the GRAVEYARD when the card's script carries a `flashback` mana cost: it
--      pays that cost instead of the printed one, and exiles the card (this engine
--      moves a non-permanent spell out of its cast zone immediately, so flashback's
--      "then exile it" is the graveyard->exile move at cast time — the card ends in
--      exile whether the spell resolves or is countered).
--   2. Tapped tokens — the create_token branch of apply_triggered_ability_effects
--      (reproduced from mig 173) honours a `tapped` flag (the 13 Zombies enter tapped).
-- (IDE T-SQL false-positives on $$ bodies — ignore.)

-- 1. cast_spell_effect + flashback (cast from graveyard for the script's flashback cost).
...
-- ===========================================================================
-- apply_triggered_ability_effects (reproduced from mig 173) — create_token now honours
-- a `tapped` flag (Army of the Damned: thirteen tapped Zombie tokens).
