-- 202605010252_selvalas_stampede — Selvala's Stampede (council's dilemma
-- voting), the LAST card of the 92-card Dragons deck.
--   • vote_wild_free parks a vote decision for the caster; each submit
--     records the vote on the stack payload and parks the next voter (seat
--     order). The last submit tallies: wild reveals from the caster's
--     library top until that many creature cards entered under them (other
--     revealed cards bottom in a random order — approximating "shuffle the
--     rest"); the free count rides the payload.
--   • put_from_hand's count may now be a stack-payload key ('free_votes');
--     zero skips the action entirely.
