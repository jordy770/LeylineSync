-- Phase 4 / F2.1b — the `prevent_damage` card effect: real spells/abilities now
-- create damage-prevention shields (F2.1a built the resolver + add_damage_prevention).
--
-- A spell_effect / triggered-ability program action:
--   { "type": "prevent_damage", "amount": 3, "combat_only": false }
-- amount omitted = prevent ALL (Fog-style). The shield protects the controller
-- ("prevent the next N damage that would be dealt to YOU this turn"); targeting an
-- opponent / a chosen player is a later slice.
--
-- prevent_damage is a NON-decision effect, so it slots into apply_trigger_effects
-- as an inline branch that calls add_damage_prevention and falls through to the
-- next effect. Reproduces apply_trigger_effects (CURRENT = mig 118) verbatim except
-- the new branch. (IDE T-SQL false-positives on $$ — ignore.)
...
begin
  ...
  while v_i < v_count loop
    ...
    if v_type in ('scry', 'surveil') then
      ...
      -- Self-shield: prevent the next N damage to the controller this turn (amount
      -- omitted = prevent all). Non-decision — create the shield inline and fall
