-- 202605010289_omen_casts
-- OMEN back-face casts (mig 289): Flush Out + Dynamic Soar.
--   • activate_ability's zone gate is now per-ability: an ability declaring
--     source_zone_required 'hand' activates from hand (battlefield stays the
--     default). Omen casts are modelled as hand abilities — the {1}{R} /
--     {2}{G} costs are real mana costs; sorcery timing unenforced as usual.
--   • shuffle_self_into_library rider: the omen card shuffles itself into
--     its owner's library after resolving (never the graveyard).
-- Approximations: Flush Out's 'if you do' draw is unconditional (an empty
-- hand skips the discard park but still draws two).
-- Generated from supabase/functions_src (activate_ability, apply_triggered_ability_effects) — those files are
