-- Proliferate (Atraxa, Praetors' Voice end step) — "Choose any number of permanents
-- and/or players that have a counter on them, then give each another counter of each
-- kind already there."
--
-- This engine models only +1/+1 counters (game_cards.plus_one_counters) and no player
-- counters, so proliferate = "choose any number of permanents that have a +1/+1
-- counter, each gets another." It's a CHOICE → a decision (reuses the multi-select
-- CardPickBody UI). Works as a triggered ability (Atraxa) and as a spell.
--
-- Two reproduced fns + one branch each:
--   * apply_trigger_effects (CURRENT = mig 126) — a `proliferate` park branch: options
--     = every battlefield permanent (ANY owner) with a counter; deciding_player = the
--     controller; min 0 (any number) / max all.
--   * submit_decision (CURRENT = mig 111) — `proliferate` validates like choose_cards
--     ({chosen:[ids]}) and applies +1 plus_one_counters to each chosen.
-- (IDE T-SQL false-positives on $$ bodies — ignore.)

-- ===========================================================================
-- apply_trigger_effects + the proliferate park branch.
-- ===========================================================================
...
begin
  ...
  while v_i < v_count loop
    ...
    if v_type in ('scry', 'surveil') then
      ...
      -- Any battlefield permanent (ANY owner) that already has a +1/+1 counter; the
      -- controller chooses any number (min 0). Each chosen one gets another counter.
      select coalesce(jsonb_agg(jsonb_build_object('game_card_id', gc.id, 'name', c.name) order by c.name, gc.id), '[]'::jsonb)
        into v_options
      from public.game_cards gc join public.cards c on c.id = gc.card_id
      where gc.session_id = p_session_id and gc.zone = 'battlefield' and gc.plus_one_counters > 0;
...
-- ===========================================================================
-- submit_decision + the proliferate apply branch (CURRENT = mig 111).
-- ===========================================================================
...
begin
  ...
  if v_decision.decision_type = 'scry' then
    ...
    -- Each chosen permanent gets another +1/+1 counter.
    update public.game_cards
    set plus_one_counters = plus_one_counters + 1
    where session_id = v_decision.session_id and id = any (v_chosen_ids);
    perform public.resume_or_finalize(v_decision.session_id, v_decision.source_stack_item_id);
