-- color_exclusion
-- NEGATIVE colour restriction on removal ("Destroy target NONBLACK creature":
-- Snuff Out, Bone Shredder, Executioner's Capsule, Shriekmaw). A new field
-- exclude_color (string or array) sits beside exclude_type_line; both are now
-- enforced at the apply_creature_effect choke point, so they finally work on the
-- SPELL, TRIGGERED and ACTIVATED removal paths alike (the target pickers never
-- checked exclude_type_line — Shriekmaw's nonartifact clause was inert until now).
-- Colours are derived from the target's mana cost via card_color_set (mig 131).
--
-- Generated from supabase/functions_src (apply_creature_effect, activate_ability) — those files are
-- the canonical current definitions; edit them, not past migrations. The helper
-- card_matches_exclusion (below) and the creature_simple builder passthrough are
-- migration-only (not in functions_src).

-- Shared negative-restriction test: TRUE when the target should be rejected —
-- its type line matches any exclude_type_line value, or its colour set contains
-- any exclude_color value. Both accept a string or a JSON array.
create or replace function public.card_matches_exclusion(
  p_session_id uuid, p_card_id uuid, p_exclude_type_line jsonb, p_exclude_color jsonb
) returns boolean
...
as $$
  select exists (
    ...
    where gc.id = p_card_id and gc.session_id = p_session_id
      and (
        case jsonb_typeof(p_exclude_type_line)
          when 'array' then exists (
            select 1 from jsonb_array_elements_text(p_exclude_type_line) t
            where nullif(t.value, '') is not null and c.type_line ilike '%' || t.value || '%')
          when 'string' then c.type_line ilike '%' || (p_exclude_type_line #>> '{}') || '%'
          else false
        end
        or
        case jsonb_typeof(p_exclude_color)
          when 'array' then exists (
            select 1 from jsonb_array_elements_text(p_exclude_color) col
            where lower(col.value) = any (public.card_color_set(c.mana_cost)))
          when 'string' then lower(p_exclude_color #>> '{}') = any (public.card_color_set(c.mana_cost))
          else false
        end
      )
  );
$$;
grant execute on function public.card_matches_exclusion(uuid, uuid, jsonb, jsonb) to anon, authenticated, service_role;
...
-- Preserve the negative-restriction fields on the creature_simple stack payload
-- (mig 414) so activated removal (Executioner's Capsule) carries exclude_color /
-- exclude_type_line through to apply_creature_effect. This builder is shared by
-- destroy/exile/bounce/tap/untap_creature and lives only in migrations (mig 105).
...
begin
  ...
  return jsonb_build_object(
    ...
    'exclude_color', p_payload -> 'exclude_color'
  ));
end;
$$;
