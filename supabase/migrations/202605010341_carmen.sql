-- 202605010341_carmen
-- Carmen, Cruel Skymarcher. (1) New permanent_sacrificed watcher event: fired from
-- every sacrifice site (edict in submit_decision; sac-self / sac-a-creature /
-- sac-artifacts costs in activate_ability) with the any-permanent type default in
-- fire_watcher_triggers. (2) return_from_graveyard gains a dynamic
-- max_mana_value_of:'source_power' cap and a `permanent` (exclude instant/sorcery)
-- filter in apply_trigger_effects.
-- Generated from supabase/functions_src (fire_watcher_triggers, activate_ability, submit_decision, apply_trigger_effects) — those files are
