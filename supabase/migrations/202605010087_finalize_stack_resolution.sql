-- Phase 1, slice 1 (prerequisite refactor): extract finalize_stack_resolution.
--
-- The tail of resolve_top_of_stack — mark the item resolved, rebuild continuous
-- effects, reset priority to the active player, check for a game finish, and
-- return the result json — is the same "finish resolving a stack item" sequence
-- that the upcoming pending-decision flow needs: a resolution-time decision
-- (scry/surveil/reveal) parks the item, and submitting the decision must resume
-- and finish it. Pulling the tail into its own function makes that reuse clean
-- (same move as put_in_graveyard / apply_creature_effect).
--
-- Behavior-preserving: resolve_top_of_stack still does exactly what it did; only
-- the final block is now a call. (IDE T-SQL diagnostics false-positive on $$.)
...
begin
  ...
  returning action_type into v_action_type;
...
-- resolve_top_of_stack reproduced from migration 086, with the inline tail
-- replaced by a finalize_stack_resolution call (and v_finish_state dropped from
-- the declarations).
