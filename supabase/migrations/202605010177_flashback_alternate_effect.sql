-- Flashback that does DIFFERENT/extra stuff than the hand cast (the "Increasing"
-- cycle: "If this was cast from a graveyard, do more/instead"). The script may
-- carry an optional `flashback_effect: { actions: [...] }` — an alternate spell
-- program that REPLACES the normal `spell_effect` when the card is cast from the
-- graveyard. The ENGINE selects the program by cast zone (it does not trust the
-- client's actions for this), so the flashback effect can't be substituted.
--
-- Reproduced verbatim from migration 176 with two additions:
--   * declare v_program
--   * before X-resolution, pick the program: a graveyard cast with a
--     flashback_effect uses its actions; otherwise the client-supplied actions.
