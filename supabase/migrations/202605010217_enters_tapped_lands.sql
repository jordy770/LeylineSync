-- Enters-tapped lands (the 7 duals in the Gisa deck: Salt Marsh, Submerged
-- Boneyard, Dismal Backwater, Jwar Isle Refuge, Temple of Deceit, Sunken
-- Hollow, Choked Estuary). Top-level script `enters_tapped` read in
-- cast_card_from_hand's land branch:
--   true — always tapped;
--   { "unless": { "count": "basic_lands_you_control", "at_least": 2 } } —
--     untapped when the count condition holds (Sunken Hollow; new
--     resolve_count_amount source);
--   { "unless": { "hand_has_type": ["Island","Swamp"] } } — untapped when your
--     hand has a matching card (Choked Estuary's reveal, auto-applied; the
--     reveal CHOICE isn't modelled — you always reveal if you can).
-- ETB lifegain/scry on these lands are ordinary triggered_abilities and need
-- nothing new. Lands entering via other paths (search_library) keep using that
-- path's own `tapped` handling.
-- Generated from supabase/functions_src (cast_card_from_hand, resolve_count_amount) — those files are
