-- Add CPU on HOSTED: seat the provisioned bot auth-user instead of a bare UUID.
--
-- add_bot_to_session (mig 319) seated `gen_random_uuid()`. That works on LOCAL
-- (relaxed FKs) but on hosted game_session_players.player_id FKs through
-- profiles to auth.users, so "Add CPU" failed with an FK violation — the exact
-- gap scripts/create-bot-user.mjs was built for (it provisions a real auth user
-- with profile username 'CPU 🤖'), except the RPC never used it.
--
-- Reproduced from mig 319; ONLY the seat-id resolution changes:
--   * a provisioned bot profile (username 'CPU 🤖') that is not yet seated in
--     this session is used when present (hosted path);
--   * seated already → a clear error instead of an FK violation;
--   * no bot profile at all → the old bare-UUID fallback (local dev unchanged).
-- The bot-runner's --watch loop plays any is_bot seat, so no runner change.
...
begin
  ...
  -- Seat id: the provisioned CPU auth-user when one exists (hosted — the FK
  -- target is real), falling back to a bare UUID for local dev without one.
  select p.id into v_bot
  from public.profiles p
  where p.username = 'CPU 🤖'
    and not exists (
  ...
  if v_bot is null then
    if exists (select 1 from public.profiles where username = 'CPU 🤖') then
      raise exception 'The CPU player is already seated in this game';
    end if;
