-- Phase 4 — player-chosen combat damage OVER-ASSIGNMENT.
--
-- Until now resolve_combat_damage auto-assigned the *minimum* lethal to each
-- blocker in damage-assignment order, then trampled any remainder. The attacking
-- player had no say. This adds an optional p_assignments input: the attacker may
-- distribute their combat damage across blockers (and, with trample, to the
-- defending player) however they like, subject to the rules-accurate constraint
-- that a blocker earlier in the order must be assigned LETHAL before any later
-- blocker — or the player — receives damage (CR 510.1c). Over-assignment (more
-- than lethal on a blocker) is therefore legal; skipping an earlier blocker is not.
--
-- p_assignments shape (attacker game_card id -> distribution):
--   { "<attacker_id>": { "blockers": [ {"blocker_card_id": "<id>", "amount": N}, ... ],
--                        "trample": M } }
-- Omitted attackers, omitted blockers, and a null p_assignments all fall back to
-- the existing auto-assignment — so every prior combat test is byte-identical and
-- this is a no-regression change. Validation runs as a pre-pass (no partial writes)
-- and applies uniformly to the first-strike and regular damage stages.
--
-- Dropped + recreated (signature gains a defaulted 2nd param) so the single
-- existing 1-arg caller resolves via the default. Verbatim from the baseline
-- except: the new param, the assignment declares, the validation pre-pass, and
-- the in-loop / trample overrides. (IDE T-SQL false-positives on $$ — ignore.)

drop function if exists public.resolve_combat_damage(uuid);
    ...
    if not v_has_blockers then
      ...
      then
        ...
        if v_trample_amount > 0 then
          update public.game_session_players
          set life_total = greatest(0, life_total - v_trample_amount)
          where session_id = p_session_id
            and player_id = v_assignment.defending_player_id;

          v_total_player_damage := v_total_player_damage + v_trample_amount;
        end if;
      end if;
    end if;
