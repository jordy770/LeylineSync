-- 202605010347_copy_token_cleanup
-- Copy-token end-step cleanup (Electroduplicate). create_copy_token stamps a
-- cleanup_at_end_step marker when except.cleanup_at_end_step is set; advance_step
-- puts those tokens into the graveyard (tokens cease) when the end step is left.
-- Generated from supabase/functions_src (create_copy_token, advance_step) — those files are
