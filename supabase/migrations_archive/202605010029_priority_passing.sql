...
add column if not exists priority_cycle_started_by uuid;

alter table public.game_turn_state
add column if not exists priority_pass_count integer not null default 0;

alter table public.game_turn_state
drop constraint if exists game_turn_state_priority_pass_count_check;

alter table public.game_turn_state
add constraint game_turn_state_priority_pass_count_check
check (priority_pass_count >= 0);

update public.game_turn_state
set
  priority_player_id = coalesce(priority_player_id, active_player_id),
  priority_pass_count = coalesce(priority_pass_count, 0)
where priority_player_id is null
   or priority_pass_count is null;
