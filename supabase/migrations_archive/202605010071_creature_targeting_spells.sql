-- Creature-targeting spells through the stack.
--
-- Adds two stack action types so burn/removal and combat tricks can be cast from
-- hand (not just applied via judge tools):
--   deal_damage_creature  payload { target_card_id, amount }
--   pump_creature         payload { target_card_id, power, toughness }
--
-- put_action_on_stack validates the target creature and pays the source's mana;
-- resolve_top_of_stack applies the effect when the spell resolves (marking damage
-- and re-checking lethal, or creating an until-end-of-turn pump). If the target
-- has left the battlefield by resolution the spell simply fizzles.
...
begin
  ...
  if p_action_type not in ('deal_damage_player', 'deal_damage_creature', 'pump_creature', 'counter_spell') then
...
-- Seed test spells that target creatures.
...
from (values
  (
    ...
    'Instant',
    '{1}{R}',
    'Lightning Strike Test deals 3 damage to any target.',
  ...
  (
    ...
    '{"actions":[{"type":"pump","power":3,"toughness":3,"target_type":"creature"}]}'
  )
