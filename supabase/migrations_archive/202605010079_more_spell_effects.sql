...
--
-- Adds six stack action types so instants/sorceries can do more than burn and
-- pump:
...
--   add_counters_creature payload { target_card_id, amount } -> +1/+1 counters
--
-- The creature-targeted types share the existing target-validation pattern
...
begin
  ...
  if v_stack_item.action_type = 'deal_damage_player' then
    ...
    -- Fizzles if the target already left the battlefield.
    v_target_card_id := nullif(v_stack_item.payload ->> 'target_card_id', '')::uuid;
    v_amount := coalesce((v_stack_item.payload ->> 'amount')::integer, 0);
...
from (values
  ...
  (
    ...
    '{"schema_version":2,"spell_effect":{"actions":[{"type":"add_counters","amount":1,"target_type":"creature"}]}}'
  )
