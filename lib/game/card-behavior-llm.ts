...
2. triggered_abilities — "when/whenever/at" abilities. Each entry: { "event": <event>, "effects": [ ... ] }.
   ...
   - proliferate: { "type": "proliferate" } — the controller chooses any number of permanents that have a +1/+1 counter (any owner), and each chosen one gets another +1/+1 counter. Use for "proliferate" (Atraxa, Praetors' Voice end-step trigger, Karn's Bastion, etc.). No fields. This engine only models +1/+1 counters, so player/poison/loyalty counters are out of scope — proliferate touches +1/+1 counters only.
