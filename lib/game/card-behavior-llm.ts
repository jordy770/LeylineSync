...
2. triggered_abilities — "when/whenever/at" abilities. Each entry: { "event": <event>, "effects": [ ... ] }.
   ...
   - add_counters: { "type": "add_counters", "amount": N } — put N +1/+1 counters on this permanent (the source). Use only for "put a +1/+1 counter on it/CARDNAME". For a non-+1/+1 counter add "counter_type": "minus_one_one" | "charge" | "quest" | "loyalty" | "generic" (omit it, or use "plus_one_one", for +1/+1). "minus_one_one" is a −1/−1 counter — it LOWERS power/toughness (a creature dropped to 0 toughness dies), and +1/+1 and −1/−1 counters on the same creature annihilate in pairs. Use it for "put a −1/−1 counter on target creature". Other bag counters (charge/quest/…) have NO P/T effect — they only exist to be referenced/proliferated. REMOVAL: a NEGATIVE amount removes counters ("remove a +1/+1 counter" → "amount": -1), or "all": true removes every counter of that kind (Hex Parasite / "remove all +1/+1 counters"). Removing +1/+1 counters can be lethal (toughness drops).
   ...
   - add_player_counters: { "type": "add_player_counters", "amount": N, "counter_type": "poison", "recipient": "each_opponent" } — put N player counters on players. counter_type ∈ poison | energy | experience; recipient defaults each_opponent (poison/infect lands on opponents). A player with 10+ poison counters loses the game. Use for "target player gets a poison counter" / proliferate-poison decks. Negative amount / "all": true removes (e.g. leeches removing poison). NOTE: infect/toxic combat damage is NOT modeled — poison only arrives via this effect (then proliferate multiplies it).
