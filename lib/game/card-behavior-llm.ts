...
2. triggered_abilities — "when/whenever/at" abilities. Each entry: { "event": <event>, "effects": [ ... ] }.
   ...
   - add_counters: { "type": "add_counters", "amount": N } — put N +1/+1 counters on this permanent (the source). Use only for "put a +1/+1 counter on it/CARDNAME". For a non-+1/+1 counter add "counter_type": "charge" | "quest" | "loyalty" | "generic" (omit it, or use "plus_one_one", for +1/+1). Bag counters (anything but +1/+1) have NO P/T effect — they only exist to be referenced/proliferated.
   - add_counters_all: { "type": "add_counters_all", "amount": N, "target_controller": "you" } — put N +1/+1 counters on each creature the controller controls. Also takes "counter_type".
   - add_player_counters: { "type": "add_player_counters", "amount": N, "counter_type": "poison", "recipient": "each_opponent" } — put N player counters on players. counter_type ∈ poison | energy | experience; recipient defaults each_opponent (poison/infect lands on opponents). A player with 10+ poison counters loses the game. Use for "target player gets a poison counter" / proliferate-poison decks. NOTE: infect/toxic combat damage is NOT modeled — poison only arrives via this effect (then proliferate multiplies it).
