...
Alternative graveyard cast: for "You may cast this from your graveyard by paying <cost> and sacrificing N creatures rather than paying its mana cost" (Scourge of Nel Toth), add top-level \`"graveyard_cast_cost": { "mana": "{B}{B}", "sacrifice_creatures": 2 }\`.

Undying: add top-level \`"undying": true\` ("When this creature dies, if it had no +1/+1 counters on it, return it to the battlefield with a +1/+1 counter" — Geralf's Mindcrusher).

Lands: "{T}: Add {U} or {B}" = TWO single-colour tap mana abilities (only one taps per untap — the Talisman pattern). "This land enters tapped" = top-level \`"enters_tapped": true\`; "…unless you control two or more basic lands" (Sunken Hollow) = \`"enters_tapped": { "unless": { "count": "basic_lands_you_control", "at_least": 2 } }\`; "you may reveal an Island or Swamp card from your hand; if you don't, enters tapped" (Choked Estuary) = \`"enters_tapped": { "unless": { "hand_has_type": ["Island", "Swamp"] } }\` (the reveal is auto-applied). ETB "you gain 1 life" / "scry 1" on lands are ordinary enters_the_battlefield triggered abilities.

Kicker: for "Kicker {cost}" add top-level \`"kicker": "{5}{B}"\` — the caster may pay it in addition to the printed cost; "if it was kicked" effects are a conditional with \`"condition": { "counters": "kicked", "of": "self", "at_least": 1 }\` (Josu Vess: kicked ETB makes eight tokens).

Top-level flags: if the card says "this spell can't be countered" (or "can't be countered by spells or abilities"), add \`"cant_be_countered": true\` at the top level of the object (alongside schema_version). It applies to any spell type. For "enters the battlefield with N +1/+1 counters", add top-level \`"enters_with_counters": { "amount": N }\`; the amount may also be a count spec or an ARRAY of count specs summed ("a +1/+1 counter for each other Zombie you control and each Zombie card in your graveyard" = { "amount": [ { "count": "creatures_you_control", "type_line": "Zombie" }, { "count": "cards_in_graveyard", "type_line": "Zombie" } ] } — Unbreathing Horde). For "if this creature would be dealt damage, prevent that damage and remove a +1/+1 counter from it", add top-level \`"damage_removes_counters": true\` (the whole damage event is prevented; one counter removed per event).

Flashback (instant/sorcery): if the card says "Flashback {cost}", add \`"flashback": "{cost}"\` at the top level (alongside schema_version, next to spell_effect). It lets the card be cast from the graveyard for that cost, then it is exiled. Model only the flashback cost here; the spell's normal effect still goes in spell_effect. Example: { "schema_version": 2, "flashback": "{7}{B}{B}{B}", "spell_effect": { "actions": [ … ] } }. If the flashback has an additional life payment ("Flashback—{1}{U}, Pay 3 life", Deep Analysis), add \`"flashback_life": 3\` alongside \`"flashback"\` — it is paid (in addition to the mana) only on the flashback cast. Example: { "schema_version": 2, "flashback": "{1}{U}", "flashback_life": 3, "spell_effect": { "actions": [ { "type": "choose_player", "filter": "any", "effects": [ { "type": "draw", "amount": 2 } ] } ] } }. If the spell does something DIFFERENT or MORE when cast from the graveyard ("If this was cast from a graveyard, …" — the "Increasing" cycle: create ten tokens instead of five, mill twice X, search for two cards instead of one), add a \`"flashback_effect": { "actions": [ … ] }\` — an alternate program (same shape as spell_effect) that REPLACES spell_effect when cast via flashback; write the FULL graveyard-cast effect in it. Example (Increasing Devotion: five tokens normally, ten on flashback): { "schema_version": 2, "flashback": "{7}{W}{W}{W}", "spell_effect": { "actions": [ { "type": "create_token", "token": "Human Token", "count": 5 } ] }, "flashback_effect": { "actions": [ { "type": "create_token", "token": "Human Token", "count": 10 } ] } }.
...
PLANESWALKERS (type_line "Planeswalker — …"): add a top-level \`"loyalty": N\` (starting loyalty) and \`"loyalty_abilities": [ { "cost": +1 | -3 | 0, "effects": [ … ] }, … ]\`. Each ability's cost adjusts loyalty (a +N adds loyalty, a −N removes it and needs at least N loyalty), is sorcery-speed and once per turn per planeswalker, and the planeswalker dies at 0 loyalty. The "effects" use the normal triggered-ability vocabulary (gain_life/lose_life/deal_damage/draw/mill/create_token/return_from_graveyard/add_counters/etc., resolved with the planeswalker as source). Example: { "loyalty": 5, "loyalty_abilities": [ { "cost": 1, "effects": [{ "type": "draw", "amount": 1 }] }, { "cost": -3, "effects": [{ "type": "return_from_graveyard", "to": "battlefield", "count": 1 }] } ] }. (Combat — attacking a planeswalker / redirecting damage to its loyalty — is not yet modelled.)
...
1. continuous_effects — static keyword abilities. Each entry: { "type": <keyword>, "affected": "source", "source_zone_required": "battlefield" }.
   ...
   - ANTHEMS / TRIBAL LORDS — a static team buff is a "pump" with "affected": "controller" ("creatures you control get +N/+N") or "affected": "all" ("all creatures get +N/+N"). For a TYPED lord, add "creature_type" to the payload to limit it to one subtype, and "exclude_source": true for "OTHER" (the lord doesn't buff itself). Example "Other Zombies you control get +1/+1": { "continuous_effects": [ { "type": "pump", "affected": "controller", "payload": { "power": 1, "toughness": 1, "creature_type": "Zombie", "exclude_source": true } } ] }. Omit exclude_source (Death Baron-style "Zombies you control …" that includes itself); omit creature_type for an all-creature anthem.
   - TYPED KEYWORD GRANTS — "Zombies you control have flying" (Eternal Skylord), "the Army you control has deathtouch" (Vizier), "Other creatures you control have <keyword>" (Vela). Same shape as a typed lord but with a KEYWORD type instead of pump: { "type": "flying", "affected": "controller", "payload": { "creature_type": "Zombie" } }. affected "controller" = yours / "all" = everyone; payload.creature_type limits it to a subtype (omit for all your creatures); exclude_source for "OTHER"; "token_only": true limits the grant to TOKEN creatures ("Zombie tokens you control have hexproof" — Gleaming Overseer). Works for the combat keywords flying/reach/deathtouch/trample/vigilance/haste/indestructible/first_strike/double_strike/menace/intimidate/hexproof. Dual-type lords (Death Baron "Skeletons you control and other Zombies you control get +1/+1 and have deathtouch") = one pump + one keyword entry PER type.
   - STATIC CAST-FROM-GRAVEYARD PERMISSION — "You may cast <type> spells from your graveyard" while this permanent is on the battlefield: { "type": "cast_from_graveyard", "affected": "controller", "payload": { "type_line": "Zombie", "once_per_turn": true } }. payload.type_line filters which cards qualify (omit for any); payload.once_per_turn limits it to once during each of your turns (Gisa and Geralf: "Once during each of your turns, you may cast a Zombie creature spell from your graveyard"). For a one-TURN grant from a resolved effect, use the grant_cast_from_graveyard EFFECT instead.
...
2. triggered_abilities — "when/whenever/at" abilities. Each entry: { "event": <event>, "effects": [ ... ] }.
   ...
   OTHER-SCOPED events (watch OTHER permanents, not just this card): use event "creature_entered" or "creature_died" with a "filter" to react to other creatures entering/dying. filter.type_line = a subtype match (e.g. "Zombie"; omit for any creature); filter.controller = "you" (default) / "opponent" / "any" (relative to THIS card's controller); filter.exclude_self = true for "another …" (this card doesn't fire off itself). The effects act on THIS card (the watcher) as source. Examples: "Whenever another Zombie you control enters, put a +1/+1 counter on ~" = { "event": "creature_entered", "filter": { "type_line": "Zombie", "controller": "you", "exclude_self": true }, "effects": [{ "type": "add_counters", "amount": 1 }] }; "Whenever a creature you control dies, you draw a card" (Midnight Reaper — includes itself, so NO exclude_self) = { "event": "creature_died", "filter": { "controller": "you" }, "effects": [{ "type": "draw", "amount": 1 }] }; "Whenever a creature you control dies, each opponent loses 1 life" = { "event": "creature_died", "filter": { "controller": "you" }, "effects": [{ "type": "lose_life", "amount": 1, "recipient": "each_opponent" }] }. Also "creature_got_counter" fires when a creature gets a +1/+1 counter (Conclave Mentor / Cathars' Crusade style) — same filter shape; e.g. "Whenever a creature you control gets a +1/+1 counter, draw a card" = { "event": "creature_got_counter", "filter": { "controller": "you" }, "effects": [{ "type": "draw", "amount": 1 }] } (only +1/+1 counters; not −1/−1 or bag counters). And "creature_left" fires whenever a creature LEAVES the battlefield to ANY zone — death, exile, or bounce (a death fires both creature_died and creature_left); use it for "whenever ~ or another creature you control leaves the battlefield" (Vela the Night-Clad — includes itself, so NO exclude_self): { "event": "creature_left", "filter": { "controller": "you" }, "effects": [{ "type": "lose_life", "amount": 1, "recipient": "each_opponent" }] }.
   ...
   COUNT-BASED AMOUNT: "amount" may also be a count relative to the controller: { "count": "creatures_you_control", "type_line"?: "Zombie" } (battlefield creatures you control, optionally one subtype — Distant Melody), { "count": "cards_in_graveyard", "type_line"?: "creature" } (cards you own in your graveyard — Lotleth Giant "damage = creatures in your graveyard"), { "count": "lands_you_control" }, or { "count": "devotion", "color": "B" } (your devotion to black = count of {B} pips on permanents you control — Gray Merchant of Asphodel). Example Gray Merchant ETB: effects [{ "type": "lose_life", "amount": { "count": "devotion", "color": "B" }, "recipient": "each_opponent" }, { "type": "gain_life", "amount": { "count": "devotion", "color": "B" }, "recipient": "controller" }].
   ...
   - create_token: { "type": "create_token", "token": <token name>, "count": N } — the controller creates N tokens. Allowed token names: ${BUILDER_TOKEN_NAMES.join(', ')}. Pick the closest match by creature type; if none matches, omit this effect. For "its controller creates a token" (Beast Within — the destroyed/targeted permanent's controller, not the caster), add "recipient": "target_controller" (only on a spell that also targets a permanent). For tokens that enter TAPPED ("create N tokens that are tapped", Army of the Damned), add "tapped": true.
   ...
   - amass: { "type": "amass", "amount": N } — "Amass N": if you control no Army, create a 0/0 black Zombie Army token, then put N +1/+1 counters on an Army you control (it grows the same Army across multiple amass effects). Use for War of the Spark Zombie cards (Lazotep Reaver, Eternal Skylord, Enter the God-Eternals). Works as a triggered-ability effect or a spell action. (Keyword grants like "Armies you control have flying" are separate continuous effects.)
   - tap_all / untap_all: { "type": "tap_all" | "untap_all", "target_controller": "you" } — tap/untap each creature the controller controls.
   - mill: { "type": "mill", "amount": N, "recipient": "controller" | "each_opponent" | "each_player" | "all_players" } — a player puts the top N cards of their library into their graveyard. Default recipient is "controller" ("mill N cards" = you); use "each_opponent" for "each opponent mills N". For "if at least one <type> card is milled this way, <then>" (Liliana, Untouched by Death +1) add "if_milled_type": "Zombie" and "then": [ ... ] — the then-effects apply once if any milled card matched the type, e.g. "then": [{ "type": "lose_life", "amount": 2, "recipient": "each_opponent" }, { "type": "gain_life", "amount": 2, "recipient": "controller" }].
   ...
   - choose_creature_type: { "type": "choose_creature_type", "effects": [ ... ] } — the controller chooses a creature type, then the inner effects apply with that type injected into any count-based amount's type_line. Use for "Choose a creature type, then …" (Distant Melody = { "type": "choose_creature_type", "effects": [ { "type": "draw", "amount": { "count": "creatures_you_control" } } ] } — draw a card for each creature of the chosen type you control). Leave the inner count's type_line OUT (it's filled in from the choice). The chosen type is ALSO injected into a pump_all's creature_type (see pump_all) for "creatures that aren't of the chosen type" cards.
   - pump_all: { "type": "pump_all", "power": -3, "toughness": -3, "scope": "all" | "controller", "creature_type": "Zombie", "exclude_type": true } — a TEMPORARY (until end of turn) P/T pump to EVERY creature matching the filter. scope "all" (default) = any controller, "controller" = creatures you control. creature_type filters by subtype; exclude_type:true = creatures NOT of that type. Use under choose_creature_type for Crippling Fear ("Choose a creature type. Each creature that isn't of the chosen type gets -3/-3 until end of turn") = { "type": "choose_creature_type", "effects": [ { "type": "pump_all", "power": -3, "toughness": -3, "scope": "all", "exclude_type": true } ] } — leave creature_type OUT (filled from the choice). For a fixed-type mass pump (no choice), set creature_type directly.
   - grant_keyword_all: { "type": "grant_keyword_all", "keyword": "menace", "scope": "all" | "controller", "creature_type": "Zombie", "includes_player": true } — a TEMPORARY (until end of turn) MASS keyword grant to every creature matching the filter. Allowed keywords: flying/reach/deathtouch/trample/vigilance/haste/indestructible/first_strike/double_strike/menace/intimidate/hexproof. scope "all" (default) = any controller, "controller" = your permanents only; creature_type filters by subtype (omit for all). includes_player:true records "YOU gain it too" (player hexproof — Lazotep Plating "You and permanents you control gain hexproof until end of turn" = { "type": "grant_keyword_all", "keyword": "hexproof", "scope": "controller", "includes_player": true }). "All Zombies gain menace until end of turn" (Lord of the Accursed's activated ability) = { "type": "grant_keyword_all", "keyword": "menace", "creature_type": "Zombie", "scope": "all" }. Works as a triggered-ability effect, a spell action, or an activated-ability effect.
   - grant_cast_from_graveyard (targeted, activated): "{1}: You may cast target creature card in a graveyard this turn" (Havengul Lich) = an activated ability { "costs": [ { "type": "mana", "amount": "{1}" } ], "effects": [ { "type": "grant_cast_from_graveyard" } ] } — the player picks the graveyard creature at activation and gets a card-specific until-end-of-turn cast permission. (The "gains all activated abilities" rider is not modelled — omit it.) The UNTARGETED form with "type_line" (Liliana's −3) grants a blanket typed permission instead.
   - choose_color: { "type": "choose_color", "anthem": { "power": 1, "toughness": 0, "scope": "controller" } } — "As ~ enters, choose a color. Creatures you control of the chosen color get +X/+Y" (Heraldic Banner). Use as an enters_the_battlefield trigger effect; the controller picks a colour and the colour-filtered anthem is registered for as long as the permanent stays. For the "{T}: Add one mana of the chosen color" line, author add_mana with "color": "any".
   - mass_destroy_reanimate_one: { "type": "mass_destroy_reanimate_one" } — "Destroy all creatures, then return a creature card put into a graveyard this way to the battlefield under your control" (Necromantic Selection). Destroys every creature (indestructible survives), then the caster picks ONE destroyed card to return under THEIR control. The "black Zombie in addition" type change and the spell's self-exile are not modelled — omit them.
   - conditional: { "type": "conditional", "condition": { "count": <source>, "type_line"?: "...", "at_least": N }, "effects": [ ... ] } — "If <state condition>, <effects>." The condition counts creatures_you_control / lands_you_control / cards_in_graveyard / creatures_died_this_turn / commanders_you_control (the Lieutenant check: "if you control your commander" = { "count": "commanders_you_control", "at_least": 1 }) and applies the inner NON-decision effects when the count is at least N. Loyal Subordinate "At the beginning of combat on your turn, if you control your commander, each opponent loses 3 life" = { "event": "beginning_of_combat", "effects": [ { "type": "conditional", "condition": { "count": "commanders_you_control", "at_least": 1 }, "effects": [ { "type": "lose_life", "amount": 3, "recipient": "each_opponent" } ] } ] }.
   ...
   - grant_cast_from_graveyard: { "type": "grant_cast_from_graveyard", "type_line": "Zombie" } — until end of turn, the controller MAY cast cards from their graveyard whose type matches "type_line" (a subtype substring; omit for any card). Use for "you may cast Zombie spells from your graveyard this turn" (Liliana, Untouched by Death −3). The cards are still cast normally (mana paid, permanents go on the stack); this only unlocks the graveyard as a legal cast zone. Today only permanent (non-instant/sorcery) cards can actually be cast this way.
...
3. activated_abilities — "{cost}: effect" abilities. Each entry: { "costs": [ ... ], "effects": [ ... ], "is_mana_ability"?: true }.
   Costs: { "type": "tap_self" } for {T}; { "type": "mana", "amount": "{2}{R}" } for a mana cost string; { "type": "energy", "amount": N } to spend N energy counters ({E}{E} = amount 2); { "type": "sacrifice_self" } to sacrifice this permanent (Commander's Sphere); { "type": "exile_from_graveyard", "type_line": "creature" } to "exile a creature card from a graveyard" (Cemetery Reaper — any graveyard; the player picks the card). Combine multiple cost entries for "{2}{B}, {T}, Exile a creature card from a graveyard: …".
   The ability has ONE effect. Supported: deal_damage ({ "type": "deal_damage", "amount": N, "target_type": <target> }), the creature-target effects destroy/exile/bounce/tap/untap/add_counters/pump/grant_keyword/gain_control (with "target_type": "creature", e.g. "{T}: Destroy target creature" = { "type": "destroy", "target_type": "creature" }), untargeted draw ({ "type": "draw", "amount": N }, e.g. "{2}: Draw a card"), and create_token ({ "type": "create_token", "token": "<name>", "count": N } — Cemetery Reaper's "Create a 2/2 black Zombie creature token"). For a MANA ability use add_mana — { "type": "add_mana", "color": "W|U|B|R|G|C", "amount": N } with "is_mana_ability": true. For "any color in your commander's color identity" (Command Tower, Arcane Signet), use "color": "commander" — the player picks an identity colour at tap time. For "any color" (Chromatic Lantern, rainbow lands), use "color": "any" — the player picks any of the five colours at tap time. Do NOT put decision effects (scry/search/sacrifice/may/choose_player) in an activated ability.
...
4. spell_effect — for Instants and Sorceries (the effect when the spell resolves): { "actions": [ ... ] }.
   ...
   - exile     { "type": "exile", "target_type": "creature" } — "exile target creature" (to its owner's exile zone).
    ...
    - counter   { "type": "counter", "target_type": "spell" }; add_mana { "type": "add_mana", "color": ..., "amount": N }. Counter riders (applied even if the target can't be countered): "controller_loses_life": N — the countered spell's controller loses N (Undermine); "surveil": N — YOU surveil N after it resolves (Sinister Sabotage = { "type": "counter", "target_type": "spell", "surveil": 1 }).
...
Rules:
- Only emit behavior the lists above support. If part of the card isn't expressible (targeted exile, non-creature trigger targets, etc.), omit that part rather than inventing a field.
