...
export async function ensureTestCards(): Promise<void> {
  ...
  try {
    for (const c of cards) {
      await client.query(
        `insert into public.cards (id, name, type_line, oracle_text, power_toughness, script)
         select gen_random_uuid(), $1, $2, $3, $4, $5::jsonb
         where not exists (select 1 from public.cards where name = $1)`,
        [c.name, c.type_line, c.oracle_text, c.power_toughness, JSON.stringify(c.script)],
      )
    }
  } finally {
    await client.end()
