...
- Produces:
  ...
  - `findGapBuys(supabase, userId, commanderIdentity: string[], gapBuckets: { bucket: ProposalBucket; shortfall: number }[]): Promise<{ bucket: ProposalBucket; buys: { oracleId: string; name: string; priceEur: number | null }[] }[]>` — per gap bucket (skip 'creatures': too generic to shop for; note this in a comment): query `co_card_tags` for that tag (weight desc, bounded overfetch 50 — buys are the cheapest among the most ON-THEME candidates; deliberate ruling: relevance first, then price, since cross-view price ordering has no FK embedding), joined to `co_card_oracle`, exclude oracle_ids the user owns (chunked availability lookup reuse), filter `fitsColorIdentity` in TS, sort by price asc nulls last then name, take 2.
