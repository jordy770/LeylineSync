...
- Targets verbatim from the spec: **99 = 62 nonland + 37 land**; bucket order ramp(10) → card_draw(10) → removal(8) → board_wipe(3) → creatures(25) → filler to 62; within-bucket sort `tagWeight + themeMatch + 2×commanderSynergy` (themeMatch: +2 tribal subtype hit, +1 keyword overlap), filler curve brake cmc>6 → −1, tie-break name asc; singleton per oracle_id, one bucket per card.
