...
- Theme boost (cap 20 total): **tribal** — extract creature subtypes of pool creatures (`typeLine.split(/[—-]/)[1]`, whitespace-split words); for each subtype owned ≥ 5 times (distinct cards), if the commander's oracle_text contains the subtype as a word (singular or naive plural `s`, case-insensitive), it is a tribal fact; take the highest-count one; boost `min(count, 30) / 30 * 15`. **Keywords** — commander keywords shared by ≥ 8 pool cards → `keywordOverlap`; +2.5 per keyword, cap 5.
